...
int main(int argc, char* argv[])
{
	...
	for(i = 0; i <= 11; i++)
		stack[i + 1] = CamN + (i << 5) + 2;  // Read channels F(2) A(i)
	//	stack[13] = CamN+0x0009;		// CLEAR LAM N()with F(9) A(0) ==> not needed with
	// F(2)A(11)
	...
	// Define Trigger Delay / LAM time out in Delay Register N(25) A(2) F(16) and enable
	// /disable LAM
	if(LAM = 0)
	{
		//  Define Trigger Delay to 100us for external LAM; bits 0 to 15 in Delay Register
		//  N(25) A(2) F(16)
	...
	{
		//  Define LAM time out to 100us for external LAM; bits 0 to 15 in Delay Register
		//  N(25) A(2) F(16)
		...
		if(DataFile != NULL)
		{
			if(ret > 0)
			{
				...
				for(j = 0; j <= data_lines; j++)
					fprintf(DataFile, "%hx\t", IntArray[j]);
			}
			else
			{
				error_count++;
