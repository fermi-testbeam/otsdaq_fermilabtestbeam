...
->Corrections by Andreas Ruben		06/22/2007
    1. CAMAC_Read Q and X response and F-filter corrected for control codes without data
return
    2. CAMAC DGG corrected 06/25/07
    3. VME and CAMAC_scaler_settings added06/26/07

->Change by Andreas Ruben		06/22/2007
    1. 8 bit VME Read added, may need check which byte should be read, not fully tested
yet!!!
...
->Change by Andreas Ruben		08/24/2009
    1. CAMACDGG: Added firmware version detection for proper setting of outputs with
FW 5.01

->Change by Andreas Ruben		04/20/2010
    1.CAMAC_block-read16(usb_dev_handle *hdev, int N, int A, int F, int loops, int
*Data)added
...
#include <usb.h>
//#include <iostream.h>
//#include "otsdaq-fermilabtestbeam/libusb-master/libusb/core.c"
...
******** xxusb_longstack_execute ************************
  ...
  Paramters:
    ...
    DataBuffer: pointer to the dual use buffer
        when calling , DataBuffer contains (unsigned short) stack data, with first word
serving as a placeholder upon successful return, DataBuffer contains (unsigned short) VME
data lDataLen: The number of bytes to be fetched from VME bus - not less than the actual
number expected, or the function will return -5 code. For stack consisting only of write
operations, lDataLen may be set to 1. timeout: The time in ms that should be spent tryimg
to write data.
  ...
  Note:
    The function must pass a pointer to an array of unsigned integer stack data, in which
the first word is left empty to serve as a placeholder. The function is intended for
executing long stacks, up to 4 MBytes long, both "write" and "read" oriented, such as
using multi-block transfer operations. Structure upon call: DataBuffer(0) = 0(don't care
place holder) DataBuffer(1) = (unsigned short)StackLength bits 0-15 DataBuffer(2) =
(unsigned short)StackLength bits 16-20 DataBuffer(3 - StackLength +2)  (unsigned short)
stack data StackLength represents the number of words following DataBuffer(1) word, thus
the total number of words is StackLength+2 Structure upon return: DataBuffer(0 -
(ReturnValue/2-1)) - (unsigned short)array of returned data when ReturnValue>0
*/

int xxusb_longstack_execute(usb_dev_handle* hDev,
                            void*           DataBuffer,
                            int             lDataLen,
                            int             timeout)
{
...
int ccusb_longstack_execute(usb_dev_handle* hDev,
                            void*           DataBuffer,
                            int             lDataLen,
                            int             timeout)
{
...
******** xxusb_bulk_read ************************
  ...
  Note:
    ...
    of an array of bytes, unsigned short integers (16 bits), or unsigned long integers (32
bits). The latter option of passing a pointer to an array of unsigned long integers is
meaningful when xxusb data buffering option is used (bit 7=128 of the global register)
that requires data 32-bit data alignment.
...
******** xxusb_bulk_write ************************
  ...
  Writes the content of an array of bytes, unsigned short integers, or unsigned long
integers to the USB port fifo; times out when the USB fifo is full (e.g., when xxusb is
busy).
  ...
  Note:
    Depending upon the actual need, the function may be used to pass to xxusb the data in
a form of an array of bytes, unsigned short integers (16 bits), or unsigned long integers
(32 bits).
*/
...
******** xxusb_usbfifo_read ************************
  ...
  Note:
    ...
    the data received. Also, it is relatively slow, as it performs extensive data
repacking. It is recommended to use xxusb_bulk_read with a pointer to an array of unsigned
short integers.
*/
			...
			if(dev->descriptor.idVendor == XXUSB_WIENER_VENDOR_ID)
			{
				...
				if(udev)
				{
					ret = usb_get_string_simple(
					    udev, dev->descriptor.iSerialNumber, string, sizeof(string));
...
short xxusb_flash_program(usb_dev_handle* hDev, char* config, short nsect)
{
	...
	pconfig       = config;
	char buf[518] = {
	    (char)0xAA, (char)0xAA, (char)0x55, (char)0x55, (char)0xA0, (char)0xA0};
...
******** VME_write_32 ************************
  ...
  Returns:
    ...
    Upon failure, a negative number
*/
short VME_write_32(usb_dev_handle* hdev,
...
******** VME_read_32 ************************
  ...
  Returns:
    ...
    Upon failure, a negative number
*/
short VME_read_32(usb_dev_handle* hdev,
                  short           Address_Modifier,
                  long            VME_Address,
                  long*           Data)
{
...
******** VME_write_16 ************************
  ...
  Returns:
    ...
    Upon failure, a negative number
*/
short VME_write_16(usb_dev_handle* hdev,
                   short           Address_Modifier,
                   long            VME_Address,
                   long            Data)
{
...
******** VME_read_16 ************************
  ...
  Returns:
    ...
    Upon failure, a negative number
*/
short VME_read_16(usb_dev_handle* hdev,
                  short           Address_Modifier,
                  long            VME_Address,
                  long*           Data)
{
...
******** VME_read_16 ************************
  ...
  Returns:
    ...
    Upon failure, a negative number
*/
short VME_read_8(usb_dev_handle* hdev,
                 short           Address_Modifier,
                 long            VME_Address,
                 long*           Data)
{
...
******** VME_BLT_read_32 ************************
  ...
  Returns:
    ...
    Upon failure, a negative number
*/
short VME_BLT_read_32(
    usb_dev_handle* hdev, short Adress_Modifier, int count, long VME_Address, long Data[])
{
	long  intbuf[1000];
	short ret;
	int   i = 0;
...
******** VME_DGG ************************
  ...
  Parameters:
    ...
    delay: 32 bit word consisting of
            ...
            upper 16 bits: Delay_coarse in steps of 81.7us between trigger and start of
gate gate: the time the gate should  stay open in steps of 12.5ns invert: is 1 if you wish
to invert the DGG channel output latch: is 1 if you wish to run the DGG channel latched

  Returns:
    Returns 1 when successful
    Upon failure, a negative number
*/
short VME_DGG(usb_dev_handle* hdev,
...
short VME_Output_settings(
    usb_dev_handle* hdev, int Channel, int code, int invert, int latch)
{
	short ret;
	//    long internal;
	long Data;

	if((Channel < 1) || (Channel > 2) || (code < 0) || (code > 7))
		return -1;
...
short VME_scaler_settings(
    usb_dev_handle* hdev, short channel, short trigger, int enable, int reset)
{
	long  Data, W_Data;
	short ret;
...
******** CAMAC_DGG ************************
  ...
  Returns:
    ...
    Upon failure, a negative number
*/
short CAMAC_DGG(usb_dev_handle* hdev,
                short           channel,
                short           trigger,
                short           output,
                int             delay,
...
short CAMAC_Output_settings(
    usb_dev_handle* hdev, int output, int code, int invert, int latch)
{
	short ret;
	long  internal;
	long  Data, W_Data;
...
short CAMAC_I(usb_dev_handle* hdev, int inhibit)
{
	...
	intbuf[0] = 1;
	if(inhibit)
		intbuf[1] = (long)(24 + 9 * 32 + 29 * 512 + 0x4000);
	else
		intbuf[1] = (long)(26 + 9 * 32 + 29 * 512 + 0x4000);
	ret = xxusb_stack_execute(hdev, intbuf);
	return ret;
}
