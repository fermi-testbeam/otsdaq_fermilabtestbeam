...
void logerror(const char* format, ...) __attribute__((format(__printf__, 1, 2)));
...
{
	fprintf(stderr,
	        "\nUsage: fxload [-v] [-V] [-t type] [-d vid:pid] [-p bus,addr] [-s loader] "
	        "-i firmware\n");
	...
	fprintf(stderr,
	        "  -p <bus,addr>   -- Target device, as a libusb bus number and device "
	        "address path\n");
...
int main(int argc, char* argv[])
{
	...
	while((opt = getopt(argc, argv, "qvV?hd:p:i:I:s:S:t:")) != EOF)
		...
		case 'p':
			...
			{
				fputs(
				    "please specify bus number & device number as \"bus,dev\" in decimal "
				    "format\n",
				    stderr);
				return -1;
			}
		...
		for(i = 0; (dev = devs[i]) != NULL; i++)
		{
			...
			if((type != NULL) && (device_path != NULL))
			{
				...
				if((libusb_get_bus_number(dev) == busnum) &&
				   (libusb_get_device_address(dev) == devaddr))
					break;
			}
			else
			{
	...
	else
	{
		...
		if(status == 0)
		{
			...
			status =
			    ezusb_load_ram(device, path[FIRMWARE], fx_type, img_type[FIRMWARE], 1);
		}
	}
