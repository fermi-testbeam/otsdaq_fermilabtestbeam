...
extern void logerror(const char* format, ...) __attribute__((format(printf, 1, 2)));
...
static int (*parse[IMG_TYPE_MAX])(FILE* image,
                                  void* context,
                                  bool (*is_external)(uint32_t addr, size_t len),
                                  int (*poke)(void*                context,
                                              uint32_t             addr,
...
static int ram_poke(
    void* context, uint32_t addr, bool external, const unsigned char* data, size_t len)
{
...
static int fx3_load_ram(libusb_device_handle* device, const char* path)
{
	...
	{
		if((ezusb_read(
		        device, "read bootloader version", RW_INTERNAL, 0xFFFF0020, blBuf, 4) <
		    0))
		{
	...
	while(1)
	{
		...
		while(dLength > 0)
		{
			...
			if(dLen > dLength)
				dLen = dLength;
			if((ezusb_write(device, "write firmware", RW_INTERNAL, dAddress, bBuf, dLen) <
			    0) ||
