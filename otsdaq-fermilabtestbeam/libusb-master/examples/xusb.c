...
// Mass Storage Requests values. See section 3 of the Bulk-Only Mass Storage Class
// specifications
...
static int display_ps3_status(libusb_device_handle* handle)
{
	...
	CALL_CHECK(libusb_control_transfer(
	    handle,
...
static int display_xbox_status(libusb_device_handle* handle)
{
	...
	printf("   Start:%d, Back:%d, Left Stick Press:%d, Right Stick Press:%d\n",
	       B(input_report[2] & 0x10),
	       B(input_report[2] & 0x20),
	       B(input_report[2] & 0x40),
                                     ...
                                     uint32_t*             ret_tag)
{
	...
	{
		perr(
		    "send_mass_storage_command: don't know how to handle this command (%02X, "
		    "length %d)\n",
		    cdb[0],
		    cdb_len);
		return -1;
                                   ...
                                   uint32_t              expected_tag)
{
	...
	{
		perr(
		    "   get_mass_storage_status: mismatched tags (expected %08X, received "
		    "%08X)\n",
		    expected_tag,
		    csw.dCSWTag);
		return -1;
...
static void get_sense(libusb_device_handle* handle,
                      uint8_t               endpoint_in,
                      uint8_t               endpoint_out)
{
	...
	rc = libusb_bulk_transfer(
	    handle, endpoint_in, (unsigned char*)&sense, REQUEST_SENSE_LENGTH, &size, 1000);
	if(rc < 0)
	{
...
static int test_mass_storage(libusb_device_handle* handle,
                             uint8_t               endpoint_in,
                             uint8_t               endpoint_out)
{
	int            r, size;
	...
	CALL_CHECK(libusb_bulk_transfer(
	    handle, endpoint_in, (unsigned char*)&buffer, INQUIRY_LENGTH, &size, 1000));
	printf("   received %d bytes\n", size);
	...
	send_mass_storage_command(handle,
	                          ...
	                          &expected_tag);
	CALL_CHECK(libusb_bulk_transfer(
	    handle, endpoint_in, (unsigned char*)&buffer, READ_CAPACITY_LENGTH, &size, 1000));
	printf("   received %d bytes\n", size);
	...
	send_mass_storage_command(
	    handle, endpoint_out, lun, cdb, LIBUSB_ENDPOINT_IN, block_size, &expected_tag);
...
static int test_hid(libusb_device_handle* handle, uint8_t endpoint_in)
{
	...
	descriptor_size = libusb_control_transfer(
	    handle,
	...
	size = get_hid_record_size(
	    hid_report_descriptor, descriptor_size, HID_REPORT_TYPE_FEATURE);
	if(size <= 0)
	{
	...
	size = get_hid_record_size(
	    hid_report_descriptor, descriptor_size, HID_REPORT_TYPE_INPUT);
	if(size <= 0)
	{
	...
	else
	{
		...
		else
		{
			...
			case LIBUSB_ERROR_TIMEOUT:
				printf(
				    "   Timeout! Please make sure you act on the device within the 5 "
				    "seconds allocated...\n");
				break;
			case LIBUSB_ERROR_PIPE:
				printf("   Detected stall - resetting pipe...\n");
		...
		// Attempt a bulk read from endpoint 0 (this should just return a raw input
		// report)
		printf("\nTesting interrupt read using endpoint %02X...\n", endpoint_in);
		r = libusb_interrupt_transfer(
		    handle, endpoint_in, report_buffer, size, &size, 5000);
		if(r >= 0)
		{
			display_buffer_hex(report_buffer, size);
		}
...
// installation
static void read_ms_winsub_feature_descriptors(libusb_device_handle* handle,
                                               uint8_t               bRequest,
                                               int                   iface_number)
{
#define MAX_OS_FD_LENGTH 256
	...
	} os_fd[2] = {{"Extended Compat ID", LIBUSB_RECIPIENT_DEVICE, 0x0004, 0x10},
	...
	for(i = 0; i < 2; i++)
	{
		...
		r = libusb_control_transfer(
		    handle,
		    (uint8_t)(LIBUSB_ENDPOINT_IN | LIBUSB_REQUEST_TYPE_VENDOR |
		              os_fd[i].recipient),
		    bRequest,
		    (uint16_t)(((iface_number) << 8) | 0x00),
		    os_fd[i].index,
		    os_desc,
		    os_fd[i].header_size,
		    1000);
		...
		{
			perr("   Failed: %s",
			     (r < 0) ? libusb_strerror((enum libusb_error)r)
...
static int test_device(uint16_t vid, uint16_t pid)
{
	...
	const char* const                        speed_name[5] = {"Unknown",
	...
	for(i = 0; i < 3; i++)
	{
		...
		if(libusb_get_string_descriptor_ascii(
		       handle, string_index[i], (unsigned char*)string, sizeof(string)) > 0)
		{
	...
	r = libusb_get_string_descriptor(handle,
	                                 MS_OS_DESC_STRING_INDEX,
	                                 0,
	                                 (unsigned char*)string,
	                                 MS_OS_DESC_STRING_LENGTH);
	if(r == MS_OS_DESC_STRING_LENGTH &&
	   memcmp(ms_os_desc_string, string, sizeof(ms_os_desc_string)) == 0)
	{
		// If this is a Microsoft OS String Descriptor,
		// attempt to read the WinUSB extended Feature Descriptors
		read_ms_winsub_feature_descriptors(
		    handle, string[MS_OS_DESC_VENDOR_CODE_OFFSET], first_iface);
	}
...
int main(int argc, char** argv)
{
		...
		for(j = 1; j < argc; j++)
		{
			...
			else
			{
				...
				if(i != arglen)
				{
					...
					{
						printf(
						    "   Please specify VID & PID as \"vid:pid\" in hexadecimal "
						    "format\n");
						return 1;
					}
	...
	{
		printf(
		    "usage: %s [-h] [-d] [-i] [-k] [-b file] [-l lang] [-j] [-x] [-s] [-p] [-w] "
		    "[vid:pid]\n",
		    argv[0]);
		...
		printf(
		    "   -w      : force the use of device requests when querying WCID "
		    "descriptors\n");
		printf(
		    "If only the vid:pid is provided, xusb attempts to run the most appropriate "
		    "test\n");
		return 0;
	}

	// xusb is commonly used as a debug tool, so it's convenient to have debug output
	// during libusb_init(), but since we can't call on libusb_set_option() before
	// libusb_init(), we use the env variable method
