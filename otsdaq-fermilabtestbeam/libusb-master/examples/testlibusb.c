...
static void print_bos(libusb_device_handle* handle)
{
	...
	if(bos->dev_capability[0]->bDevCapabilityType == LIBUSB_BT_USB_2_0_EXTENSION)
	{
		...
		ret = libusb_get_usb_2_0_extension_descriptor(
		    NULL, bos->dev_capability[0], &usb_2_0_extension);
		if(0 > ret)
		{
			return;
		}
	...
	if(bos->dev_capability[0]->bDevCapabilityType == LIBUSB_BT_SS_USB_DEVICE_CAPABILITY)
	{
		...
		ret = libusb_get_ss_usb_device_capability_descriptor(
		    NULL, bos->dev_capability[0], &dev_cap);
		if(0 > ret)
		{
			return;
		}
...
static int print_device(libusb_device* dev, int level)
{
	...
	if(LIBUSB_SUCCESS == ret)
	{
		...
		{
			ret = libusb_get_string_descriptor_ascii(
			    handle, desc.iProduct, string, sizeof(string));
			if(ret > 0)
