...
char* getenv(const char* name)
{
	...
	rc =
	    RegOpenKeyEx(HKEY_LOCAL_MACHINE, ENVIRONMENT_REG_PATH, 0, KEY_QUERY_VALUE, &hkey);
	if(rc != ERROR_SUCCESS)
	{
	...
	rc     = RegQueryValueEx(hkey, wName, NULL, &dwType, (LPBYTE)&wValue, &dwData);
	RegCloseKey(hkey);
	if(rc != ERROR_SUCCESS)
	{
	...
	if(WideCharToMultiByte(
	       CP_UTF8, 0, wValue, dwData / sizeof(*wValue), value, MAX_PATH, NULL, NULL) <=
	   0)
	{
