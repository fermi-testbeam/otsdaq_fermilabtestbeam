 ...
 * functions that retrieve various \ref libusb_desc "USB descriptors". These functions
must
...
int main (void) {
  ...
  rc = libusb_hotplug_register_callback(NULL, LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED |
                                        LIBUSB_HOTPLUG_EVENT_DEVICE_LEFT, 0, 0x045a,
0x5005, LIBUSB_HOTPLUG_MATCH_ANY, hotplug_callback, NULL, &callback_handle); if
(LIBUSB_SUCCESS != rc) { printf("Error creating a hotplug callback\n"); libusb_exit(NULL);
    return EXIT_FAILURE;
  }
	   ...
	   (events & LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED))
	{
		...
		for(i = 0; i < len; i++)
		{
			usbi_hotplug_match_cb(
			    ctx, devs[i], LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED, new_callback);
		}
...
void API_EXPORTED libusb_hotplug_deregister_callback(
    struct libusb_context* ctx, libusb_hotplug_callback_handle callback_handle)
{
	struct libusb_hotplug_callback* hotplug_cb;
	int                             deregistered = 0;
	...
	usbi_mutex_lock(&ctx->hotplug_cbs_lock);
	list_for_each_entry(
	    hotplug_cb, &ctx->hotplug_cbs, list, struct libusb_hotplug_callback)
	{
...
void usbi_hotplug_deregister(struct libusb_context* ctx, int forced)
{
	    ...
	    hotplug_cb, next, &ctx->hotplug_cbs, list, struct libusb_hotplug_callback)
	{
		...
		{
			usbi_dbg(
			    "freeing hotplug cb %p with handle %d", hotplug_cb, hotplug_cb->handle);
			list_del(&hotplug_cb->list);
			free(hotplug_cb);
		}
	}
