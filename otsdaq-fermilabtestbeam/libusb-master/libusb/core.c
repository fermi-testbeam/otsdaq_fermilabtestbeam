...
//#include "otsdaq_fermilabtestbeam/libusb-master/config.h"
...
static const struct libusb_version libusb_version_internal = {LIBUSB_MAJOR,
                                                              LIBUSB_MINOR,
                                                              LIBUSB_MICRO,
                                                              LIBUSB_NANO,
                                                              LIBUSB_RC,
                                                    ...
                                                    unsigned long          session_id)
{
	...
	usbi_mutex_lock(&ctx->usb_devs_lock);
	list_for_each_entry(
	    dev, &ctx->usb_devs, list, struct libusb_device) if(dev->session_data ==
	                                                        session_id)
	{
...
static void do_close(struct libusb_context* ctx, struct libusb_device_handle* dev_handle)
{
	...
	list_for_each_entry_safe(
	    itransfer, tmp, &ctx->flying_transfers, list, struct usbi_transfer)
	{
		...
		{
			usbi_err(ctx,
			         "Device handle closed while transfer was still being processed, but "
			         "the device is still connected as far as we know");

			if(itransfer->state_flags & USBI_TRANSFER_CANCELLING)
				usbi_warn(ctx,
				          "A cancellation for an in-flight transfer hasn't completed but "
				          "closing the device handle");
			else
				usbi_err(ctx,
				         "A cancellation hasn't even been scheduled on the transfer for "
				         "which the device is closing");
		}
                                                  ...
                                                  int                   alternate_setting)
{
	...
	return usbi_backend.set_interface_altsetting(
	    dev_handle, interface_number, alternate_setting);
}
                                      ...
                                      int                   num_endpoints)
{
	...
	if(usbi_backend.alloc_streams)
		return usbi_backend.alloc_streams(
		    dev_handle, num_streams, endpoints, num_endpoints);
	else
 ...
 */
int API_EXPORTED libusb_set_auto_detach_kernel_driver(libusb_device_handle* dev_handle,
                                                      int                   enable)
{
