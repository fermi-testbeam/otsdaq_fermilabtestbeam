                               ...
                               int                              host_endian)
{
	...
	for(i = 0; i < config->bNumInterfaces; i++)
	{
		...
		while(size >= DESC_HEADER_LENGTH)
		{
			...
			{
				usbi_warn(
				    ctx, "short extra config desc read %d/%d", size, header.bLength);
 ...
 */
int API_EXPORTED libusb_get_active_config_descriptor(
    libusb_device* dev, struct libusb_config_descriptor** config)
{
	...
	r = usbi_backend.get_active_config_descriptor(
	    dev, tmp, LIBUSB_DT_CONFIG_SIZE, &host_endian);
	if(r < 0)
		return r;
	if(r < LIBUSB_DT_CONFIG_SIZE)
	{
		usbi_err(
		    dev->ctx, "short config descriptor read %d/%d", r, LIBUSB_DT_CONFIG_SIZE);
		return LIBUSB_ERROR_IO;
	}
                                   ...
                                   int*                  idx)
{
	...
	for(i = 0; i < dev->num_configurations; i++)
	{
		...
		int           host_endian;
		int           r =
		    usbi_backend.get_config_descriptor(dev, i, tmp, sizeof(tmp), &host_endian);
		if(r < 0)
		{
			*idx = -1;
			return r;
                                           ...
                                           struct libusb_bos_descriptor** bos)
{
	...
	r = libusb_get_descriptor(
	    dev_handle, LIBUSB_DT_BOS, 0, bos_header, LIBUSB_DT_BOS_SIZE);
	if(r < 0)
	{
    ...
    struct libusb_usb_2_0_extension_descriptor** usb_2_0_extension)
{
	...
	usbi_parse_descriptor(
	    (unsigned char*)dev_cap, "bbbd", _usb_2_0_extension, host_endian);
    ...
    struct libusb_ss_usb_device_capability_descriptor** ss_usb_device_cap)
{
	...
	usbi_parse_descriptor(
	    (unsigned char*)dev_cap, "bbbbwbbw", _ss_usb_device_cap, host_endian);
