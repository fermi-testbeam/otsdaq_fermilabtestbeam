...
static int  sunos_get_device_descriptor(struct libusb_device*, uint8_t*, int*);
static int  sunos_get_active_config_descriptor(struct libusb_device*,
                                               uint8_t*,
                                               size_t,
                                               int*);
static int  sunos_get_config_descriptor(
     struct libusb_device*, uint8_t, uint8_t*, size_t, int*);
...
static int  sunos_set_interface_altsetting(struct libusb_device_handle*, int, int);
...
static int  sunos_detach_kernel_driver(struct libusb_device_handle* dev,
                                       int                          interface_number);
static int  sunos_attach_kernel_driver(struct libusb_device_handle* dev,
                                       int                          interface_number);
...
static int                   sunos_get_link(di_devlink_t devlink, void* arg)
{
	...
	if(larg->path)
	{
		...
		if(start == NULL || strncmp(start, larg->path, larg->len) != 0 ||
		   start[larg->len] != ':')
...
static int sunos_physpath_to_devlink(const char* node_path,
                                     const char* match,
                                     char**      link_path)
{
	walk_link_t         larg;
	...
	*link_path  = NULL;
	...
	(void)di_devlink_walk(
	    hdl, match, NULL, DI_PRIMARY_LINK, (void*)&larg, sunos_get_link);
...
static int sunos_usb_ioctl(struct libusb_device* dev, int cmd)
{
	...
	nvlist_pack(
	    nvlist, (char**)&iocdata.nvl_user, &iocdata.nvl_usersz, NV_ENCODE_NATIVE, 0);
...
static int sunos_init(struct libusb_context* ctx) { return (LIBUSB_SUCCESS); }

static void sunos_exit(struct libusb_context* ctx) { usbi_dbg(""); }
...
static string_list_t* sunos_new_string_list(void)
{
	...
	if(list != NULL)
		list_create(list, sizeof(string_node_t), offsetof(string_node_t, link));
...
static int sunos_exec_command(struct libusb_context* ctx,
                              const char*            path,
                              string_list_t*         list)
{
	pid_t  pid;
...
static int sunos_detach_kernel_driver(struct libusb_device_handle* dev_handle,
                                      int                          interface_number)
{
...
static int sunos_attach_kernel_driver(struct libusb_device_handle* dev_handle,
                                      int                          interface_number)
{
	struct libusb_context* ctx = HANDLE_CTX(dev_handle);
	string_list_t*         list;
...
static int sunos_fill_in_dev_info(di_node_t node, struct libusb_device* dev)
{
	...
	proplen =
	    di_prop_lookup_bytes(DDI_DEV_T_ANY, node, "usb-raw-cfg-descriptors", &rdata);
	if(proplen <= 0)
	{
	...
	usbi_dbg(
	    "vid=%x pid=%x, path=%s, bus_nmber=0x%x, port_number=%d, "
	    "speed=%d",
	    dpriv->dev_descr.idVendor,
	    dpriv->dev_descr.idProduct,
	    dpriv->phypath,
...
static void sunos_usb_close_ep0(sunos_dev_handle_priv_t* hdev, sunos_dev_priv_t* dpriv)
{
...
static int sunos_find_interface(struct libusb_device_handle* hdev,
                                uint8_t                      endpoint,
                                uint8_t*                     interface)
{
...
static int sunos_check_device_and_status_open(struct libusb_device_handle* hdl,
                                              uint8_t                      ep_addr,
                                              int                          ep_type)
{
	...
	{
		usbi_dbg("ep 0x%02x already opened, return success", ep_addr);
	...
	{
		usbi_dbg("can't find interface for endpoint 0x%02x", ep_addr);
	...
	if((ep_type == LIBUSB_TRANSFER_TYPE_INTERRUPT) && (ep_addr & LIBUSB_ENDPOINT_IN))
	{
		...
		{
			usbi_dbg("can't open %s RDWR: %d", statfilename, errno);
		}
		else
		{
			...
			if(count != 1)
			{
				...
				usbi_dbg("can't write to %s: %d", statfilename, errno);
				(void)close(fdstat);
	...
	{
		usbi_dbg("can't open %s: %d(%s)", filename, errno, strerror(errno));
...
int sunos_get_device_descriptor(struct libusb_device* dev, uint8_t* buf, int* host_endian)
{
	sunos_dev_priv_t* dpriv = (sunos_dev_priv_t*)dev->os_priv;
...
int sunos_get_config_descriptor(
    struct libusb_device* dev, uint8_t idx, uint8_t* buf, size_t len, int* host_endian)
{
	/* XXX */
	return (sunos_get_active_config_descriptor(dev, buf, len, host_endian));
}

int sunos_get_configuration(struct libusb_device_handle* handle, int* config)
{
...
int sunos_claim_interface(struct libusb_device_handle* handle, int iface)
{
...
int sunos_release_interface(struct libusb_device_handle* handle, int iface)
{
	sunos_dev_handle_priv_t* hpriv = (sunos_dev_handle_priv_t*)handle->os_priv;
...
int sunos_set_interface_altsetting(struct libusb_device_handle* handle,
                                   int                          iface,
                                   int                          altsetting)
{
	sunos_dev_priv_t*        dpriv = (sunos_dev_priv_t*)handle->dev->os_priv;
...
static void usb_dump_data(unsigned char* data, size_t size)
{
	int i;
...
static void sunos_async_callback(union sigval arg)
{
	...
	{
		xfer->actual_length = LIBUSB_TRANSFER_TO_USBI_TRANSFER(xfer)->transferred =
		    aio_return(aiocb);
	}

	usb_dump_data(xfer->buffer, xfer->actual_length);

	usbi_dbg("ret=%d, len=%d, actual_len=%d", ret, xfer->length, xfer->actual_length);
...
static int usb_do_io(int fd, int stat_fd, char* data, size_t size, int flag, int* status)
{
	int error;
	int ret = -1;
	...
	if(ret < 0)
	{
		...
		usbi_dbg("TID=%x io %s errno=%d(%s) ret=%d",
		         pthread_self(),
		         flag ? "WRITE" : "READ",
		         errno,
		         strerror(errno),
		...
		usbi_dbg("io status=%d errno=%d(%s)", error, save_errno, strerror(save_errno));
...
int sunos_clear_halt(struct libusb_device_handle* handle, uint8_t endpoint)
{
	int ret;
	...
	ret = libusb_control_transfer(
	    handle,
	    LIBUSB_ENDPOINT_OUT | LIBUSB_RECIPIENT_ENDPOINT | LIBUSB_REQUEST_TYPE_STANDARD,
...
int sunos_reset_device(struct libusb_device_handle* handle)
{
	usbi_dbg("");
...
int sunos_cancel_transfer(struct usbi_transfer* itransfer)
{
	...
	usbi_dbg("aio->fd=%d fd=%d ret = %d, %s",
	         aiocb->aio_fildes,
	         hpriv->eps[ep].datafd,
	         ret,
...
void sunos_clear_transfer_priv(struct usbi_transfer* itransfer)
{
	usbi_dbg("");
...
int sunos_clock_gettime(int clkid, struct timespec* tp)
{
