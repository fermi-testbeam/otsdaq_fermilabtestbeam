...
// Sub-APIs for WinUSB-like driver APIs (WinUSB, libusbK, libusb-win32 through the libusbK
// DLL) Must have the same values as the KUSB_DRVID enum from libusbk.h
...
struct windows_usb_api_backend
{
	...
	const char* const* const
	              driver_name_list;  // Driver name, without .sys, e.g. "usbccgp"
	const uint8_t nb_driver_names;
	int (*init)(struct libusb_context* ctx);
	void (*exit)(void);
	int (*open)(int sub_api, struct libusb_device_handle* dev_handle);
	void (*close)(int sub_api, struct libusb_device_handle* dev_handle);
	int (*configure_endpoints)(int                          sub_api,
	                           struct libusb_device_handle* dev_handle,
	                           int                          iface);
	int (*claim_interface)(int                          sub_api,
	                       struct libusb_device_handle* dev_handle,
	                       int                          iface);
	int (*set_interface_altsetting)(int                          sub_api,
	                                ...
	                                int                          iface,
	                                int                          altsetting);
	int (*release_interface)(int                          sub_api,
	                         struct libusb_device_handle* dev_handle,
	                         int                          iface);
	int (*clear_halt)(int                          sub_api,
	                  struct libusb_device_handle* dev_handle,
	...
	int (*copy_transfer_data)(int                   sub_api,
...
static inline struct winusb_device_priv* winusb_device_priv_init(
...
DLL_DECLARE_FUNC_PREFIXED(WINAPI,
                          LONG,
                          p,
                          RegQueryValueExW,
...
DLL_DECLARE_FUNC_PREFIXED(
    WINAPI, HDEVINFO, p, SetupDiGetClassDevsA, (LPCGUID, PCSTR, HWND, DWORD));
DLL_DECLARE_FUNC_PREFIXED(
    WINAPI, BOOL, p, SetupDiEnumDeviceInfo, (HDEVINFO, DWORD, PSP_DEVINFO_DATA));
DLL_DECLARE_FUNC_PREFIXED(
    WINAPI,
    BOOL,
    p,
...
typedef BOOL(WINAPI* WinUsb_AbortPipe_t)(WINUSB_INTERFACE_HANDLE InterfaceHandle,
                                         UCHAR                   PipeID);
...
typedef BOOL(WINAPI* WinUsb_FlushPipe_t)(WINUSB_INTERFACE_HANDLE InterfaceHandle,
                                         UCHAR                   PipeID);
...
typedef BOOL(WINAPI* WinUsb_ResetPipe_t)(WINUSB_INTERFACE_HANDLE InterfaceHandle,
                                         UCHAR                   PipeID);
typedef BOOL(WINAPI* WinUsb_SetCurrentAlternateSetting_t)(
    WINUSB_INTERFACE_HANDLE InterfaceHandle, UCHAR AlternateSetting);
...
typedef BOOL(WINAPI* LibK_GetProcAddress_t)(PVOID* ProcAddress,
                                            ULONG  DriverID,
                                            ULONG  FunctionID);

typedef VOID(WINAPI* LibK_GetVersion_t)(PKLIB_VERSION Version);

// KISO_PACKET is equivalent of libusb_iso_packet_descriptor except uses absolute "offset"
// field instead of sequential Lengths
...
// KISO_CONTEXT is the conceptual equivalent of libusb_transfer except is
// isochronous-specific and must match libusbk's version
...
typedef BOOL(WINAPI* WinUsb_IsoReadPipe_t)(WINUSB_INTERFACE_HANDLE InterfaceHandle,
                                           UCHAR                   PipeID,
                                           PUCHAR                  Buffer,
                                           ULONG                   BufferLength,
                                           LPOVERLAPPED            Overlapped,
...
typedef BOOL(WINAPI* WinUsb_IsoWritePipe_t)(WINUSB_INTERFACE_HANDLE InterfaceHandle,
                                            UCHAR                   PipeID,
                                            PUCHAR                  Buffer,
                                            ULONG                   BufferLength,
                                            LPOVERLAPPED            Overlapped,
