...
int linux_netlink_start_event_monitor(void)
{
	...
	{
		usbi_dbg("failed to create netlink socket of type %d, attempting SOCK_RAW",
		         socktype);
		socktype             = SOCK_RAW;
		linux_netlink_socket = socket(PF_NETLINK, socktype, NETLINK_KOBJECT_UEVENT);
	}
	...
	ret = pthread_create(
	    &libusb_linux_event_thread, NULL, linux_netlink_event_thread_main, NULL);
	if(ret != 0)
	{
...
static int linux_netlink_read_message(void)
{
	...
	if(cred->uid != 0)
	{
		usbi_dbg("ignoring netlink message with non-zero sender UID %u",
		         (unsigned int)cred->uid);
		return -1;
	}

	r = linux_netlink_parse(
	    msg_buffer, (size_t)len, &detached, &sys_name, &busnum, &devaddr);
	if(r)
		return r;

	usbi_dbg(
	    "netlink hotplug found device busnum: %hhu, devaddr: %hhu, sys_name: %s, "
	    "removed: %s",
	    busnum,
	    devaddr,
	    sys_name,
