...
static CFRunLoopSourceRef libusb_darwin_acfls =
    NULL; /* shutdown signal for event cf loop */

static usbi_mutex_t     darwin_cached_devices_lock = PTHREAD_MUTEX_INITIALIZER;
static struct list_head darwin_cached_devices      = {&darwin_cached_devices,
                         ...
                         struct darwin_interface**    interface_out)
{
	...
	usbi_warn(
	    HANDLE_CTX(dev_handle), "no pipeRef found with endpoint address 0x%02x.", ep);
...
static int usb_setup_device_iterator(io_iterator_t* deviceIterator, UInt32 location)
{
	...
	if(location)
	{
		...
		/* there are no unsigned CFNumber types so treat the value as signed. the OS seems
		 to do this internally (CFNumberType of locationID is kCFNumberSInt32Type) */
		...
		if(propertyMatchDict && locationCF)
		{
			...
			CFDictionarySetValue(
			    matchingDict, CFSTR(kIOPropertyMatchKey), propertyMatchDict);
		}
		/* else we can still proceed as long as the caller accounts for the possibility of
		 * other devices in the iterator */
	...
	return IOServiceGetMatchingServices(
	    kIOMasterPortDefault, matchingDict, deviceIterator);
}

/* Returns 1 on success, 0 on failure. */
static int get_ioregistry_value_number(io_service_t service,
                                       CFStringRef  property,
                                       CFNumberType type,
                                       void*        p)
{
	CFTypeRef cfNumber =
	    IORegistryEntryCreateCFProperty(service, property, kCFAllocatorDefault, 0);
	int ret = 0;
...
static int get_ioregistry_value_data(io_service_t service,
                                     CFStringRef  property,
                                     ssize_t      size,
                                     void*        p)
{
...
static usb_device_t** darwin_device_from_service(io_service_t service)
{
	...
	(void)(*plugInInterface)
	    ->QueryInterface(
	        plugInInterface, CFUUIDGetUUIDBytes(DeviceInterfaceID), (LPVOID)&device);
	/* Use release instead of IODestroyPlugInInterface to avoid stopping IOServices
...
static void darwin_devices_detached(void* ptr, io_iterator_t rem_devices)
{
	...
	while((device = IOIteratorNext(rem_devices)) != 0)
	{
		...
		ret = get_ioregistry_value_number(
		    device, CFSTR("sessionID"), kCFNumberSInt64Type, &session);
		IOObjectRelease(device);
		if(!ret)
			continue;

		/* we need to match darwin_ref_cached_device call made in darwin_get_cached_device
	   function otherwise no cached device will ever get freed */
		usbi_mutex_lock(&darwin_cached_devices_lock);
		list_for_each_entry(
		    old_device, &darwin_cached_devices, list, struct darwin_cached_device)
		{
		...
		list_for_each_entry(ctx, &active_contexts_list, list, struct libusb_context)
		{
			...
			if(dev)
			{
				/* signal the core that this device has been disconnected. the core will
		   tear down this device when the reference count reaches 0 */
...
static void* darwin_event_thread_main(void* arg0)
{
	...
	{
		usbi_err(
		    ctx, "could not add hotplug event source: %s", darwin_error_str(kresult));
...
static void __attribute__((destructor)) _darwin_finalize(void)
{
	...
	list_for_each_entry_safe(
	    dev, next, &darwin_cached_devices, list, struct darwin_cached_device)
	{
                                        ...
                                        int*                  host_endian)
{
	...
	kresult =
	    (*priv->device)->GetConfigurationDescriptorPtr(priv->device, config_index, &desc);
	if(kresult == kIOReturnSuccess)
	{
		/* copy descriptor */
                                      ...
                                      struct darwin_cached_device* dev)
{
	...
	/* checking the configuration of a root hub simulation takes ~1 s in 10.11. the device
	...
	/* check if the device is already configured. there is probably a better way than
	 iterating over the to accomplish this (the trick is we need to avoid a call to
	 GetConfigurations since buggy devices might lock up on the device request) */
	...
	kresult = (*(darwin_device))
	              ->CreateInterfaceIterator(darwin_device, &request, &interface_iterator);
	if(kresult)
		return darwin_to_libusb(kresult);
	...
	usbi_dbg(
	    "active config: %u, first config: %u", dev->active_config, dev->first_config);
                                          ...
                                          struct darwin_cached_device* dev)
{
	...
	/* According to Apple's documentation the device must be open for DeviceRequest but we
	 * may not be able to open some devices and Apple's USB Prober doesn't bother to open
	 * the device before issuing a descriptor request.  Still, to follow the spec as
	 * closely as possible, try opening the device */
	...
	do
	{
		...
		if(kIOReturnOverrun == ret &&
		   kUSBDeviceDesc == dev->dev_descriptor.bDescriptorType)
		                               ...
		                               0 == dev->dev_descriptor.bcdUSB))
		{
			...
			{
				usbi_dbg(
				    "descriptor appears to be invalid. resetting configuration before "
				    "trying again...");
		...
		{
			usbi_dbg(
			    "kernel responded with code: 0x%08x. sleeping for %d ms before trying "
			    "again",
			    ret,
			    delay / 1000);
			/* sleep for a little while before trying again */
			nanosleep(&(struct timespec){delay / 1000000, (delay * 1000) % 1000000000UL},
			          NULL);
		}
	} while(kIOReturnSuccess != ret && retries--);

	if(unsuspended) /* resuspend the device */
	...
	if(ret != kIOReturnSuccess)
	{
		...
		if(LIBUSB_CLASS_HUB == bDeviceClass)
			usbi_dbg(
			    "could not retrieve device descriptor %.4x:%.4x: %s (%x). skipping "
			    "device",
			    idVendor,
			    idProduct,
	...
	if(libusb_le16_to_cpu(dev->dev_descriptor.idProduct) != idProduct)
	{
		/* not a valid device */
		usbi_warn(ctx,
		          "idProduct from iokit (%04x) does not match idProduct in descriptor "
		          "(%04x). skipping device",
		          idProduct,
                                    ...
                                    struct darwin_cached_device** cached_out)
{
	...
	(void)get_ioregistry_value_number(
	    service, CFSTR("sessionID"), kCFNumberSInt64Type, &sessionID);
	...
	do
	{
		*cached_out = NULL;
		...
		list_for_each_entry(
		    new_device, &darwin_cached_devices, list, struct darwin_cached_device)
		{
	   ...
	   which may be used by claim_interface) */
		...
		if(cached_device->parent_session > 0)
		{
			dev->parent_dev = usbi_get_device_by_session_id(
			    ctx, (unsigned long)cached_device->parent_session);
		}
		else
		{
			dev->parent_dev = NULL;
...
static int darwin_open(struct libusb_device_handle* dev_handle)
{
	...
	if(0 == dpriv->open_count)
	{
		...
		{
			usbi_warn(
			    HANDLE_CTX(dev_handle), "USBDeviceOpen: %s", darwin_error_str(kresult));
			...
			/* it is possible to perform some actions on a device that is not open so do
			 * not return an error */
			priv->is_open = 0;
		}
		else
	...
	if(0 == dpriv->open_count)
	{
		...
		if(priv->cfSource)
		{
			CFRunLoopRemoveSource(
			    libusb_darwin_acfl, priv->cfSource, kCFRunLoopDefaultMode);
...
static int darwin_get_interface(usb_device_t** darwin_device,
                                uint8_t        ifc,
                                io_service_t*  usbInterfacep)
{
	...
	*usbInterfacep = IO_OBJECT_NULL;
	...
	while((*usbInterfacep = IOIteratorNext(interface_iterator)))
	{
		...
		ret = get_ioregistry_value_number(*usbInterfacep,
...
static int get_endpoints(struct libusb_device_handle* dev_handle, int iface)
{
	...
	for(int i = 1; i <= numep; i++)
	{
		...
		if(kresult != kIOReturnSuccess)
		{
			/* probably a buggy device. try to get the endpoint address from the
			 * descriptors */
			...
			{
				usbi_err(HANDLE_CTX(dev_handle),
				         "can't get alternate setting for interface");
				return darwin_to_libusb(kresult);
			}
			...
			endpoint_desc =
			    config->interface[iface].altsetting[alt_setting].endpoint + i - 1;
		...
		else
		{
			cInterface->endpoint_addrs[i - 1] =
			    (((kUSBIn == direction) << kUSBRqDirnShift) |
...
static int darwin_claim_interface(struct libusb_device_handle* dev_handle, int iface)
{
	...
	{
		usbi_err(
		    HANDLE_CTX(dev_handle), "USBInterfaceOpen: %s", darwin_error_str(kresult));
		return darwin_to_libusb(kresult);
	}
...
static int darwin_release_interface(struct libusb_device_handle* dev_handle, int iface)
{
	...
	{
		CFRunLoopRemoveSource(
		    libusb_darwin_acfl, cInterface->cfSource, kCFRunLoopDefaultMode);
	...
	if(kresult)
		usbi_warn(
		    HANDLE_CTX(dev_handle), "USBInterfaceClose: %s", darwin_error_str(kresult));
...
static int darwin_set_interface_altsetting(struct libusb_device_handle* dev_handle,
                                           int                          iface,
                                           int                          altsetting)
{
	struct darwin_device_handle_priv* priv =
                             ...
                             unsigned char                endpoint)
{
	...
	if(kresult)
		usbi_warn(
		    HANDLE_CTX(dev_handle), "ClearPipeStall: %s", darwin_error_str(kresult));
...
static int darwin_reset_device(struct libusb_device_handle* dev_handle)
{
	...
	do
	{
		...
		/* ignore return code. if we can't get a descriptor it might be worthwhile
		 * re-enumerating anway */
		...
		{
			usbi_dbg(
			    "darwin/reset_device: checking if configuration descriptor %d changed",
			    i);
...
static int submit_bulk_transfer(struct usbi_transfer* itransfer)
{
	...
	if(ep_to_pipeRef(
	       transfer->dev_handle, transfer->endpoint, &pipeRef, NULL, &cInterface) != 0)
	{
...
static int submit_iso_transfer(struct usbi_transfer* itransfer)
{
	...
	/* copy the frame list from the libusb descriptor (the structures differ only is
	 * member order) */
	...
	if(LIBUSB_SPEED_FULL == transfer->dev_handle->dev->speed) /* Full speed */
...
static int submit_control_transfer(struct usbi_transfer* itransfer)
{
	...
	if(transfer->endpoint)
	{
		...
		if(ep_to_pipeRef(
		       transfer->dev_handle, transfer->endpoint, &pipeRef, NULL, &cInterface) !=
		   0)
		{
			usbi_err(TRANSFER_CTX(transfer), "endpoint not found on any open interface");
...
static int darwin_abort_transfers(struct usbi_transfer* itransfer)
{
	...
	if(ep_to_pipeRef(
	       transfer->dev_handle, transfer->endpoint, &pipeRef, &iface, &cInterface) != 0)
	{
		usbi_err(TRANSFER_CTX(transfer), "endpoint not found on any open interface");
...
static void darwin_async_io_callback(void* refcon, IOReturn result, void* arg0)
{
	   ...
	   transfer->flags & LIBUSB_TRANSFER_ADD_ZERO_PACKET)
	{
		...
		(*(cInterface->interface))
		    ->WritePipe(cInterface->interface, pipeRef, transfer->buffer, 0);
	}
...
static int darwin_handle_transfer_completion(struct usbi_transfer* itransfer)
{
	...
	return usbi_handle_transfer_completion(
	    itransfer, darwin_transfer_status(itransfer, tpriv->result));
}
                                ...
                                int                          num_endpoints)
{
	...
	for(i = 0; i < num_endpoints; ++i)
	{
		...
		rc = (*(cInterface->interface))
		         ->CreateStreams(cInterface->interface, pipeRef, num_streams);
