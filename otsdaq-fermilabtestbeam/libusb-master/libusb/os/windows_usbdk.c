...
static inline struct usbdk_transfer_priv* _usbdk_transfer_priv(
...
static int load_usbdk_helper_dll(struct libusb_context* ctx)
{
	...
	usbdk_helper.GetDevicesList =
	    (USBDK_GET_DEVICES_LIST)get_usbdk_proc_addr(ctx, "UsbDk_GetDevicesList");
	if(usbdk_helper.GetDevicesList == NULL)
		goto error_unload;

	usbdk_helper.ReleaseDevicesList =
	    (USBDK_RELEASE_DEVICES_LIST)get_usbdk_proc_addr(ctx, "UsbDk_ReleaseDevicesList");
	if(usbdk_helper.ReleaseDevicesList == NULL)
		goto error_unload;

	usbdk_helper.StartRedirect =
	    (USBDK_START_REDIRECT)get_usbdk_proc_addr(ctx, "UsbDk_StartRedirect");
	if(usbdk_helper.StartRedirect == NULL)
		goto error_unload;

	usbdk_helper.StopRedirect =
	    (USBDK_STOP_REDIRECT)get_usbdk_proc_addr(ctx, "UsbDk_StopRedirect");
	if(usbdk_helper.StopRedirect == NULL)
		goto error_unload;

	usbdk_helper.GetConfigurationDescriptor =
	    (USBDK_GET_CONFIGURATION_DESCRIPTOR)get_usbdk_proc_addr(
	...
	usbdk_helper.ReleaseConfigurationDescriptor =
	    (USBDK_RELEASE_CONFIGURATION_DESCRIPTOR)get_usbdk_proc_addr(
	...
	usbdk_helper.WritePipe =
	    (USBDK_WRITE_PIPE)get_usbdk_proc_addr(ctx, "UsbDk_WritePipe");
	if(usbdk_helper.WritePipe == NULL)
		goto error_unload;

	usbdk_helper.AbortPipe =
	    (USBDK_ABORT_PIPE)get_usbdk_proc_addr(ctx, "UsbDk_AbortPipe");
	if(usbdk_helper.AbortPipe == NULL)
		goto error_unload;

	usbdk_helper.ResetPipe =
	    (USBDK_RESET_PIPE)get_usbdk_proc_addr(ctx, "UsbDk_ResetPipe");
	if(usbdk_helper.ResetPipe == NULL)
		goto error_unload;

	usbdk_helper.SetAltsetting =
	    (USBDK_SET_ALTSETTING)get_usbdk_proc_addr(ctx, "UsbDk_SetAltsetting");
	if(usbdk_helper.SetAltsetting == NULL)
		goto error_unload;

	usbdk_helper.ResetDevice =
	    (USBDK_RESET_DEVICE)get_usbdk_proc_addr(ctx, "UsbDk_ResetDevice");
	if(usbdk_helper.ResetDevice == NULL)
		goto error_unload;

	usbdk_helper.GetRedirectorSystemHandle =
	    (USBDK_GET_REDIRECTOR_SYSTEM_HANDLE)get_usbdk_proc_addr(
...
static int usbdk_init(struct libusb_context* ctx)
{
	...
	{
		usbi_warn(
		    ctx, "failed to open service control manager: %s", windows_error_str(0));
                                           ...
                                           unsigned long*         session_id)
{
	...
	if(snprintf(
	       dev_identity, sizeof(dev_identity), "%S%S", id->DeviceID, id->InstanceID) ==
	   -1)
	{
                                          ...
                                          PUSB_DK_DEVICE_INFO       info)
{
	...
	for(i = 0; i < info->DeviceDescriptor.bNumConfigurations; i++)
	{
		...
		Request.Index = i;
		if(!usbdk_helper.GetConfigurationDescriptor(
		       &Request, &p->config_descriptors[i], &Length))
		{
...
static int usbdk_get_config_descriptor(struct libusb_device* dev,
...
static int usbdk_get_config_descriptor_by_value(struct libusb_device* dev,
...
static int usbdk_get_active_config_descriptor(struct libusb_device* dev,
                                              ...
                                              size_t                len)
{
	return usbdk_get_config_descriptor(
	    dev, _usbdk_device_priv(dev)->active_configuration, buffer, len);
}
...
static int usbdk_clear_halt(struct libusb_device_handle* dev_handle,
...
static int usbdk_do_control_transfer(struct usbi_transfer* itransfer)
{
	...
	if(transfer->buffer[0] & LIBUSB_ENDPOINT_IN)
		transResult = usbdk_helper.ReadPipe(
		    priv->redirector_handle, &transfer_priv->request, overlapped);
	else
		transResult = usbdk_helper.WritePipe(
		    priv->redirector_handle, &transfer_priv->request, overlapped);
	...
	case TransferSuccess:
		windows_force_sync_completion(
		    overlapped, (ULONG)transfer_priv->request.Result.GenResult.BytesTransferred);
		break;
...
static int usbdk_do_bulk_transfer(struct usbi_transfer* itransfer)
{
	...
	default:
		usbi_err(
		    ctx, "Wrong transfer type (%d) in usbdk_do_bulk_transfer", transfer->type);
		return LIBUSB_ERROR_INVALID_PARAM;
	}

	if(IS_XFERIN(transfer))
		transferRes = usbdk_helper.ReadPipe(
		    priv->redirector_handle, &transfer_priv->request, overlapped);
	else
		transferRes = usbdk_helper.WritePipe(
		    priv->redirector_handle, &transfer_priv->request, overlapped);
...
static int usbdk_do_iso_transfer(struct usbi_transfer* itransfer)
{
	...
	transfer_priv->IsochronousPacketsArray =
	    malloc(transfer->num_iso_packets * sizeof(ULONG64));
	transfer_priv->request.IsochronousPacketsArray =
	    (PVOID64)transfer_priv->IsochronousPacketsArray;
	...
	transfer_priv->IsochronousResultsArray =
	    malloc(transfer->num_iso_packets * sizeof(USB_DK_ISO_TRANSFER_RESULT));
	transfer_priv->request.Result.IsochronousResultsArray =
	    (PVOID64)transfer_priv->IsochronousResultsArray;
...
static int usbdk_submit_transfer(struct usbi_transfer* itransfer)
{
	...
	case LIBUSB_TRANSFER_TYPE_INTERRUPT:
		if(IS_XFEROUT(transfer) && (transfer->flags & LIBUSB_TRANSFER_ADD_ZERO_PACKET))
			return LIBUSB_ERROR_NOT_SUPPORTED;  // TODO: Check whether we can support this
			                                    // in UsbDk
...
static void usbdk_get_overlapped_result(struct usbi_transfer* itransfer,
                                        DWORD*                io_result,
                                        DWORD*                io_size)
{
	struct usbdk_transfer_priv* transfer_priv = _usbdk_transfer_priv(itransfer);
	...
	if(HasOverlappedIoCompletedSync(
	       pollable_fd
	           ->overlapped)  // Handle async requests that completed synchronously first
	   || GetOverlappedResult(
	          transfer_priv->system_handle, pollable_fd->overlapped, io_size, FALSE))
	{  // Regular async overlapped
		...
		if(transfer->type == LIBUSB_TRANSFER_TYPE_ISOCHRONOUS)
		{
			...
			for(i = 0; i < transfer_priv->request.IsochronousPacketsArraySize; i++)
			{
				...
				lib_desc->actual_length =
				    (unsigned int)transfer_priv->IsochronousResultsArray[i].ActualLength;
			}
		}

		*io_size   = (DWORD)transfer_priv->request.Result.GenResult.BytesTransferred;
		*io_result = usbdk_translate_usbd_status(
		    (USBD_STATUS)transfer_priv->request.Result.GenResult.UsbdStatus);
	}
	else
	{
