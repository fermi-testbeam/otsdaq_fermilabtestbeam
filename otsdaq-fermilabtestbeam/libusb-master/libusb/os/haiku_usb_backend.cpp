...
int _errno_to_libusb(int status) { return status; }
...
struct usbi_transfer* USBTransfer::UsbiTransfer() { return fUsbiTransfer; }

void USBTransfer::SetCancelled() { fCancelled = true; }

bool USBTransfer::IsCancelled() { return fCancelled; }
...
void USBTransfer::Do(int fRawFD)
{
	...
	{
		usb_raw_command command;
		command.transfer.interface =
		    fUSBDevice->EndpointToInterface(fLibusbTransfer->endpoint);
		command.transfer.endpoint =
		    fUSBDevice->EndpointToIndex(fLibusbTransfer->endpoint);
		...
		{
			if(ioctl(
			       fRawFD, B_USB_RAW_COMMAND_BULK_TRANSFER, &command, sizeof(command)) ||
	...
	{
		usb_raw_command command;
		command.isochronous.interface =
		    fUSBDevice->EndpointToInterface(fLibusbTransfer->endpoint);
		command.isochronous.endpoint =
		    fUSBDevice->EndpointToIndex(fLibusbTransfer->endpoint);
		...
		usb_iso_packet_descriptor* packetDescriptors =
		    new usb_iso_packet_descriptor[fLibusbTransfer->num_iso_packets];
		for(i = 0; i < fLibusbTransfer->num_iso_packets; i++)
		{
			if((int16)(fLibusbTransfer->iso_packet_desc[i]).length !=
			   (fLibusbTransfer->iso_packet_desc[i]).length)
			{
				fUsbiTransfer->transferred = -1;
			...
			packetDescriptors[i].request_length =
			    (int16)(fLibusbTransfer->iso_packet_desc[i]).length;
		}
		...
		for(i = 0; i < fLibusbTransfer->num_iso_packets; i++)
		{
			(fLibusbTransfer->iso_packet_desc[i]).actual_length =
...
bool USBDeviceHandle::InitCheck() { return fInitCheck; }
...
USBDeviceHandle::USBDeviceHandle(USBDevice* dev)
    : fTransfersThread(-1), fUSBDevice(dev), fClaimedInterfaces(0), fInitCheck(false)
{
	...
	fTransfersThread =
	    spawn_thread(TransfersThread, "Transfer Worker", B_NORMAL_PRIORITY, this);
...
int USBDeviceHandle::ClaimInterface(int inumber)
{
...
int USBDeviceHandle::ReleaseInterface(int inumber)
{
...
int USBDeviceHandle::SetConfiguration(int config)
{
...
int USBDeviceHandle::SetAltSetting(int inumber, int alt)
{
	usb_raw_command command;
	...
	if(ioctl(fRawFD, B_USB_RAW_COMMAND_SET_ALT_INTERFACE, &command, sizeof(command)) ||
	   command.alternate.status != B_USB_RAW_STATUS_SUCCESS)
	{  // IF IOCTL FAILS DEVICE DISONNECTED PROBABLY
...
bool USBDevice::InitCheck() { return fInitCheck; }

const char* USBDevice::Location() const { return fPath; }
...
const usb_device_descriptor* USBDevice::Descriptor() const { return &fDeviceDescriptor; }

const usb_configuration_descriptor* USBDevice::ConfigurationDescriptor(uint32 index) const
{
...
int USBDevice::ActiveConfigurationIndex() const { return fActiveConfiguration; }
...
int USBDevice::CheckInterfacesFree(int config)
{
...
int USBDevice::SetActiveConfiguration(int config_index)
{
...
int USBDevice::Initialise()  // Do we need more error checking, etc? How to report?
{
	...
	if(ioctl(
	       fRawFD, B_USB_RAW_COMMAND_GET_DEVICE_DESCRIPTOR, &command, sizeof(command)) ||
	...
	fConfigurationDescriptors =
	    new(std::nothrow) unsigned char*[fDeviceDescriptor.num_configurations];
	fEndpointToIndex =
	    new(std::nothrow) map<uint8, uint8>[fDeviceDescriptor.num_configurations];
	fEndpointToInterface =
	    new(std::nothrow) map<uint8, uint8>[fDeviceDescriptor.num_configurations];
	for(int i = 0; i < fDeviceDescriptor.num_configurations; i++)
	{
		...
		fConfigurationDescriptors[i] =
		    new(std::nothrow) unsigned char[tmp_config.total_length];
