...
static int  winusbx_configure_endpoints(int                          sub_api,
                                        struct libusb_device_handle* dev_handle,
                                        int                          iface);
static int  winusbx_claim_interface(int                          sub_api,
                                    struct libusb_device_handle* dev_handle,
                                    int                          iface);
static int  winusbx_release_interface(int                          sub_api,
                                      struct libusb_device_handle* dev_handle,
                                      int                          iface);
...
static int  hid_claim_interface(int                          sub_api,
                                struct libusb_device_handle* dev_handle,
                                int                          iface);
static int  hid_release_interface(int                          sub_api,
                                  struct libusb_device_handle* dev_handle,
                                  int                          iface);
...
static int  hid_clear_halt(int                          sub_api,
                           struct libusb_device_handle* dev_handle,
                           unsigned char                endpoint);
...
static int  composite_claim_interface(int                          sub_api,
                                      struct libusb_device_handle* dev_handle,
                                      int                          iface);
static int  composite_set_interface_altsetting(int                          sub_api,
                                               ...
                                               int                          iface,
                                               int                          altsetting);
static int  composite_release_interface(int                          sub_api,
                                        struct libusb_device_handle* dev_handle,
                                        int                          iface);
static int  composite_submit_control_transfer(int                   sub_api,
                                              struct usbi_transfer* itransfer);
                             ...
                             unsigned               _index)
{
	if(_index == 0)
	{
		*dev_info = pSetupDiGetClassDevsA(
		    NULL, enumerator, NULL, DIGCF_PRESENT | DIGCF_ALLCLASSES);
 ...
 * incremented index starting at zero) until all interfaces have been returned.
 */
static int get_interface_details(struct libusb_context* ctx,
                                 HDEVINFO               dev_info,
                                 ...
                                 char**                 dev_interface_path)
{
	...
	for(;;)
	{
		if(!pSetupDiEnumDeviceInfo(dev_info, *_index, dev_info_data))
		{
			...
			{
				usbi_err(ctx,
				         "Could not obtain device info data for %s index %u: %s",
				         guid_to_string(guid),
				         *_index,
				         windows_error_str(0));
		...
		if(pSetupDiEnumDeviceInterfaces(
		       dev_info, dev_info_data, guid, 0, &dev_interface_data))
			break;

		if(GetLastError() != ERROR_NO_MORE_ITEMS)
		{
			usbi_err(ctx,
			         "Could not obtain interface data for %s devInst %X: %s",
			         guid_to_string(guid),
	       ...
	       dev_info, &dev_interface_data, NULL, 0, &size, NULL))
	{
		...
		{
			usbi_err(ctx,
			         "could not access interface data (dummy) for %s devInst %X: %s",
			         guid_to_string(guid),
			         dev_info_data->DevInst,
			         windows_error_str(0));
			return LIBUSB_ERROR_OTHER;
	...
	else
	{
		usbi_err(ctx,
		         ...
		         "http://msdn.microsoft.com/en-us/library/ms792901.aspx is wrong");
		return LIBUSB_ERROR_OTHER;
	}
	...
	{
		usbi_err(ctx,
		         "could not allocate interface data for %s devInst %X",
		         guid_to_string(guid),
		         dev_info_data->DevInst);
	...
	if(!pSetupDiGetDeviceInterfaceDetailA(
	       dev_info, &dev_interface_data, dev_interface_details, size, NULL, NULL))
	{
		usbi_err(ctx,
		         "could not access interface data (actual) for %s devInst %X: %s",
		         guid_to_string(guid),
		         dev_info_data->DevInst,
		         windows_error_str(0));
	...
	*dev_interface_path = sanitize_path(dev_interface_details->DevicePath);
	...
	{
		usbi_err(ctx,
		         "could not allocate interface path for %s devInst %X",
		         guid_to_string(guid),
		         dev_info_data->DevInst);
		return LIBUSB_ERROR_NO_MEM;
	}
...
/* For libusb0 filter */
static SP_DEVICE_INTERFACE_DETAIL_DATA_A* get_interface_details_filter(
    ...
    char*                  filter_path)
{
	...
	{
		HKEY hkey_device_interface = pSetupDiOpenDeviceInterfaceRegKey(
		    *dev_info, &dev_interface_data, 0, KEY_READ);
		if(hkey_device_interface != INVALID_HANDLE_VALUE)
		{
			...
			if(status == ERROR_SUCCESS)
			{
				if(libusb0_symboliclink_index < 256)
				{
					...
					sprintf(filter_path,
					        "\\\\.\\libusb0-%04u",
					        (unsigned int)libusb0_symboliclink_index);
					usbi_dbg("assigned libusb0 symbolic link %s", filter_path);
				}
				else
				{
					// libusb0.sys was connected to this device instance at one time; but
					// not anymore.
				}
			}
 ...
 */
static int get_interface_by_endpoint(struct libusb_config_descriptor* conf_desc,
                                     uint8_t                          ep)
{
	...
	for(i = 0; i < conf_desc->bNumInterfaces; i++)
	{
		...
		for(j = 0; j < intf->num_altsetting; j++)
		{
			...
			for(k = 0; k < intf_desc->bNumEndpoints; k++)
			{
				...
				{
					usbi_dbg("found endpoint %02X on interface %d",
 ...
 */
static int auto_claim(struct libusb_transfer* transfer,
                      ...
                      int                     api_type)
{
	...
	{  // No serviceable interface was found
		for(current_interface = 0; current_interface < USB_MAXINTERFACES;
		    current_interface++)
		{
			...
			if((priv->usb_interface[current_interface].apib->id == api_type) &&
			   (libusb_claim_interface(transfer->dev_handle, current_interface) ==
			    LIBUSB_SUCCESS))
			{
				usbi_dbg("auto-claimed interface %d for control request",
				         current_interface);
...
static int winusb_init(struct libusb_context* ctx)
{
	...
	for(i = 0; i < USB_API_MAX; i++)
	{
		if(usb_api_backend[i].init && usb_api_backend[i].init(ctx))
			usbi_warn(
			    ctx, "error initializing %s backend", usb_api_backend[i].designation);
	}
...
static void cache_config_descriptors(struct libusb_device* dev, HANDLE hub_handle)
{
	...
	{
		usbi_err(ctx,
		         "could not allocate configuration descriptor array for '%s'",
		         priv->dev_id);
		return;
	}
	...
	for(i = 0; i <= dev->num_configurations; i++)
	{
		...
		cd_buf_short.req.SetupPacket.wLength =
		    (USHORT)sizeof(USB_CONFIGURATION_DESCRIPTOR);

		// Dummy call to get the required data size. Initial failures are reported as info
		// rather than error as they can occur for non-penalizing situations, such as with
		// some hubs. coverity[tainted_data_argument]
		...
		{
			usbi_info(ctx,
			          "could not access configuration descriptor %u (dummy) for '%s': %s",
			          i,
			          priv->dev_id,
			          windows_error_str(0));
			continue;
		...
		if((ret_size != size) ||
		   (cd_buf_short.desc.wTotalLength < sizeof(USB_CONFIGURATION_DESCRIPTOR)))
		{
			usbi_info(ctx,
			          "unexpected configuration descriptor %u size (dummy) for '%s'",
			          i,
			          priv->dev_id);
			continue;
		}
		...
		{
			usbi_err(ctx,
			         "could not allocate configuration descriptor %u buffer for '%s'",
			         i,
			         priv->dev_id);
			continue;
		}
		...
		{
			usbi_err(ctx,
			         "could not access configuration descriptor %u (actual) for '%s': %s",
			         i,
			         priv->dev_id,
			         windows_error_str(0));
			continue;
		...
		if((size != ret_size) ||
		   (cd_data->wTotalLength != cd_buf_short.desc.wTotalLength))
		{
			usbi_err(ctx,
			         "unexpected configuration descriptor %u size (actual) for '%s'",
			         i,
			         priv->dev_id);
			continue;
		}
		...
		{
			usbi_err(ctx,
			         "descriptor %u not a configuration descriptor for '%s'",
			         i,
			         priv->dev_id);
			continue;
		}
                       ...
                       DEVINST               devinst)
{
	...
	{  // Not a HCD root hub
		...
		{
			usbi_err(ctx,
			         "program assertion failed - bus number not found for '%s'",
			         priv->dev_id);
			return LIBUSB_ERROR_NOT_FOUND;
		}
		...
		hub_handle = CreateFileA(parent_priv->path,
		                         GENERIC_WRITE,
		                         FILE_SHARE_WRITE,
		                         NULL,
		                         OPEN_EXISTING,
		...
		usbi_dbg("found %u configurations (active conf: %u)",
		...
		// In their great wisdom, Microsoft decided to BREAK the USB speed report between
                         ...
                         int*                   sub_api)
{
	...
	for(k = 0; k < 3; k++)
	{
		                                      ...
		                                      &size))
		{
			...
			// MULTI_SZ is a pain to work with. Turn it into something much more
			// manageable NB: none of the driver names we check against contain
			// LIST_SEPARATOR, (currently ';'), so even if an unsuported one does, it's
			// not an issue
		...
		for(k = 0; k < 3; k++)
		{
			...
			if(j >= 0)
			{
				usbi_dbg("matched %s name against %s",
				         lookup[k].designation,
				         (i != USB_API_WINUSBX) ? usb_api_backend[i].designation
				                                : usb_api_backend[i].driver_name_list[j]);
				*api     = i;
				*sub_api = j;
				return;
			}
                                   ...
                                   int                    sub_api)
{
	...
	{
		usbi_warn(ctx,
		          "failure to read interface number for %s, using default value",
		          device_id);
		interface_number = 0;
	}
	...
	{
		usbi_warn(ctx,
		          "interface %d too large - ignoring interface path %s",
		          interface_number,
		          dev_interface_path);
		return LIBUSB_ERROR_ACCESS;
	}
                             ...
                             char*                  dev_interface_path)
{
	...
	for(i = 0; i < priv->hid->nb_interfaces; i++)
	{
		if((priv->usb_interface[i].path != NULL) &&
		   strcmp(priv->usb_interface[i].path, dev_interface_path) == 0)
		{
...
#define UNREF_SIZE_STEP 16
	...
	dev_info_intf = pSetupDiGetClassDevsA(
	    NULL, NULL, NULL, DIGCF_ALLCLASSES | DIGCF_PRESENT | DIGCF_DEVICEINTERFACE);
...
//#define ENUM_DEBUG
		...
		usbi_dbg("#### PROCESSING %ss %s",
		         passname[MIN(pass, EXT_PASS)],
		         guid_to_string(guid_list[pass]));
#endif
		...
		for(i = 0;; i++)
		{
			...
			{
				usbi_warn(ctx,
				          "program assertion failed - found more than %u buses, skipping "
				          "the rest.",
				          UINT8_MAX);
				break;
			}
			...
			if(!pSetupDiGetDeviceInstanceIdA(
			       *dev_info, &dev_info_data, dev_id, sizeof(dev_id), NULL))
			{
				usbi_warn(
				    ctx,
				    "could not read the device instance ID for devInst %X, skipping",
				    dev_info_data.DevInst);
				continue;
			}
			...
			case HUB_PASS:
				...
				if(j == nb_usb_enumerators)
				{
					...
					if(nb_usb_enumerators < ARRAYSIZE(usb_enumerator))
					{
						...
						{
							usbi_err(ctx,
							         "could not allocate enumerator string '%s'",
							         enumerator);
					...
					{
						usbi_warn(ctx,
						          "too many enumerator strings, some devices may not be "
						          "accessible");
					}
				}
				break;
			case GEN_PASS:
				...
				if(!pSetupDiGetDeviceRegistryPropertyA(
				       *dev_info, &dev_info_data, SPDRP_DRIVER, NULL, NULL, 0, NULL) &&
				...
				if(key == INVALID_HANDLE_VALUE)
					break;
				// Look for both DeviceInterfaceGUIDs *and* DeviceInterfaceGUID, in that
				// order
				...
				if((s == ERROR_SUCCESS) &&
				   (((reg_type == REG_SZ) &&
				     (size == (sizeof(guid_string_w) - sizeof(WCHAR)))) ||
				    ((reg_type == REG_MULTI_SZ) && (size == sizeof(guid_string_w)))))
				{
					if(nb_guids == guid_size)
					{
						new_guid_list =
						    realloc((void*)guid_list,
						            (guid_size + GUID_SIZE_STEP) * sizeof(void*));
					...
					{
						usbi_warn(ctx,
						          "device '%s' has malformed DeviceInterfaceGUID string, "
						          "skipping",
						          dev_id);
						free(if_guid);
					}
				...
				{
					usbi_warn(ctx,
					          "unexpected type/size of DeviceInterfaceGUID for '%s'",
					          dev_id);
				}
				break;
			case HID_PASS:
			...
			default:
				...
				{
					usbi_warn(ctx,
					          "driver for device '%s' is reporting an issue (code: %u) - "
					          "skipping",
					          dev_id,
			...
			if(pass >= GEN_PASS)
			{
				...
				if(parent_dev == NULL)
				{
					...
					dev = usbi_get_device_by_session_id(
					    ctx, (unsigned long)dev_info_data.DevInst);
					if(dev != NULL)
					{
						priv = _device_priv(dev);
						if(priv->root_hub)
					...
					usbi_dbg(
					    "unlisted ancestor for '%s' (non USB HID, newly connected, etc.) "
					    "- ignoring",
					    dev_id);
					continue;
				}
				...
				parent_priv = _device_priv(parent_dev);
				// virtual USB devices are also listed during GEN - don't process these
				// yet
			...
			{  // For subsequent passes, we'll lookup the parent
				...
				else
				{
					...
					{
						usbi_dbg("device instance ID for session [%lX] changed",
						         session_id);
			...
			track_unref:
				...
				{
					new_unref_list = realloc(
					    unref_list, (unref_size + UNREF_SIZE_STEP) * sizeof(void*));
			...
			case DEV_PASS:
				...
				default:
					...
					if(priv->usb_interface[0].path == NULL)
						LOOP_BREAK(LIBUSB_ERROR_NO_MEM);
					// The following is needed if we want API calls to work for both
				...
				break;
			case HCD_PASS:
				r = enumerate_hcd_root_hub(
				    ctx, dev_id, (uint8_t)(i + 1), dev_info_data.DevInst);
				break;
			case GEN_PASS:
			...
			default:  // HID_PASS and later
				if(parent_priv->apib->id == USB_API_HID ||
				   parent_priv->apib->id == USB_API_COMPOSITE)
				{
					if(parent_priv->apib->id == USB_API_HID)
					{
					...
					else
					{
						usbi_dbg("setting composite interface for [%lX]:",
						         parent_dev->session_data);
						r = set_composite_interface(
						    ctx, parent_dev, dev_interface_path, dev_id, api, sub_api);
					}
					switch(r)
					{
					case LIBUSB_SUCCESS:
					...
					case LIBUSB_ERROR_ACCESS:
						// interface has already been set => make sure dev_interface_path
						// is freed then
						r = LIBUSB_SUCCESS;
						break;
					default:
...
static int winusb_get_config_descriptor(struct libusb_device* dev,
...
static int winusb_get_config_descriptor_by_value(struct libusb_device* dev,
                                           ...
                                           int                          altsetting)
{
	...
	r = priv->apib->set_interface_altsetting(
	    SUB_API_NOTSET, dev_handle, iface, altsetting);
...
static int winusb_clear_halt(struct libusb_device_handle* dev_handle,
                             unsigned char                endpoint)
{
	struct winusb_device_priv* priv = _device_priv(dev_handle->dev);
...
static int do_submit_transfer(struct usbi_transfer* itransfer,
                              short                 events,
                                         ...
                                         DWORD*                io_size)
{
	...
	if(HasOverlappedIoCompletedSync(pollable_fd->overlapped))
	{
		...
		*io_size   = (DWORD)pollable_fd->overlapped->InternalHigh;
	            ...
	            transfer_priv->handle, pollable_fd->overlapped, io_size, FALSE))
	{
...
const struct windows_usb_api_backend usb_api_backend[USB_API_MAX] = {
    {
        USB_API_UNSUPPORTED, "Unsupported API",
...
static int winusbx_init(struct libusb_context* ctx)
{
	...
	else
	{
		...
		pLibK_GetProcAddress =
		    (LibK_GetProcAddress_t)GetProcAddress(h, "LibK_GetProcAddress");
                                       ...
                                       int                          iface)
{
	...
	for(i = -1; i < priv->usb_interface[iface].nb_endpoints; i++)
	{
		...
		if(!WinUSBX[sub_api].SetPipePolicy(winusb_handle,
		                                   ...
		                                   sizeof(UCHAR),
		                                   &policy))
			usbi_dbg("failed to disable IGNORE_SHORT_PACKETS for endpoint %02X",
			         endpoint_address);
		...
		if(!WinUSBX[sub_api].SetPipePolicy(winusb_handle,
		                                   ...
		                                   sizeof(UCHAR),
		                                   &policy))
			usbi_dbg("failed to enable ALLOW_PARTIAL_READS for endpoint %02X",
			         endpoint_address);

		if(!WinUSBX[sub_api].SetPipePolicy(
		       winusb_handle, endpoint_address, AUTO_CLEAR_STALL, sizeof(UCHAR), &policy))
			usbi_dbg("failed to enable AUTO_CLEAR_STALL for endpoint %02X",
			         endpoint_address);
	}
                                   ...
                                   int                          iface)
{
	...
	// If the device is composite, but using the default Windows composite parent driver
	// (usbccgp) or if it's the first WinUSB-like interface, we get a handle through
	// Initialize().
	if((is_using_usbccgp) || (iface == 0))
	{
		...
		if(!WinUSBX[sub_api].Initialize(file_handle, &winusb_handle))
		{
			...
			case ERROR_BAD_COMMAND:
				...
				usbi_err(ctx,
				         "could not access interface %d: %s",
				         iface,
				         windows_error_str(0));
				return LIBUSB_ERROR_NO_DEVICE;
			default:
				...
				for(i = 0;; i++)
				{
					...
					if(strncmp(dev_path_no_guid,
					           priv->usb_interface[iface].path,
					           strlen(dev_path_no_guid)) == 0)
					{
						file_handle =
						...
						if(file_handle != INVALID_HANDLE_VALUE)
						{
							if(WinUSBX[sub_api].Initialize(file_handle, &winusb_handle))
							{
								...
								CloseHandle(
								    handle_priv->interface_handle[iface].dev_handle);
								handle_priv->interface_handle[iface].dev_handle =
								    file_handle;
								found_filter = true;
							}
							...
							{
								usbi_err(ctx,
								         "could not initialize filter driver for %s",
								         filter_path);
	...
	else
	{
		...
		if(!HANDLE_VALID(winusb_handle))
		{
			...
			if(WinUSBX[sub_api].Initialize(file_handle, &winusb_handle))
			{
				...
				usbi_warn(ctx,
				          "auto-claimed interface 0 (required to claim %d with WinUSB)",
				          iface);
			}
			else
			{
				usbi_warn(ctx,
				          "failed to auto-claim interface 0 (required to claim %d with "
				          "WinUSB): %s",
				          iface,
				          windows_error_str(0));
				return LIBUSB_ERROR_ACCESS;
		       ...
		       &handle_priv->interface_handle[iface].api_handle))
		{
			...
			default:
				usbi_err(
				    ctx, "could not claim interface %d: %s", iface, windows_error_str(0));
				return LIBUSB_ERROR_ACCESS;
			}
		}
	}
...
static int winusbx_submit_control_transfer(int sub_api, struct usbi_transfer* itransfer)
{
	...
	{
		if(!WinUSBX[sub_api].ControlTransfer(winusb_handle,
		                                     *setup,
...
static int winusbx_submit_iso_transfer(int sub_api, struct usbi_transfer* itransfer)
{
	...
	usbi_dbg(
	    "matched endpoint %02X with interface %d", transfer->endpoint, current_interface);
	...
	iso_ctx_size =
	    sizeof(KISO_CONTEXT) + (transfer->num_iso_packets * sizeof(KISO_PACKET));
	...
	if(IS_XFERIN(transfer))
	{
		...
		ret = WinUSBX[sub_api].IsoReadPipe(winusb_handle,
		                                   transfer->endpoint,
		                                   transfer->buffer,
		                                   transfer->length,
		                                   overlapped,
	...
	else
	{
		...
		ret = WinUSBX[sub_api].IsoWritePipe(winusb_handle,
		                                    transfer->endpoint,
		                                    transfer->buffer,
		                                    transfer->length,
		                                    overlapped,
...
static int winusbx_submit_bulk_transfer(int sub_api, struct usbi_transfer* itransfer)
{
	...
	if(IS_XFERIN(transfer))
	{
		...
		ret = WinUSBX[sub_api].ReadPipe(winusb_handle,
		                                transfer->endpoint,
		                                transfer->buffer,
		                                transfer->length,
		                                NULL,
	...
	else
	{
		...
		ret = WinUSBX[sub_api].WritePipe(winusb_handle,
		                                 transfer->endpoint,
		                                 transfer->buffer,
		                                 transfer->length,
		                                 NULL,
...
static int winusbx_abort_transfers(int sub_api, struct usbi_transfer* itransfer)
{
	...
	if(WinUSBX[sub_api].CancelIoEx_supported)
	{
		...
		usbi_warn(ctx,
		          "CancelIoEx not supported for sub API %s",
		          winusbx_driver_names[sub_api]);
 ...
 */
// TODO: (post hotplug): see if we can force eject the device and redetect it (reuse
// hotplug?)
static int winusbx_reset_device(int sub_api, struct libusb_device_handle* dev_handle)
{
	struct libusb_context*            ctx         = DEVICE_CTX(dev_handle->dev);
	...
	for(i = 0; i < USB_MAXINTERFACES; i++)
	{
		...
		if(HANDLE_VALID(winusb_handle))
		{
			for(j = 0; j < priv->usb_interface[i].nb_endpoints; j++)
			{
				...
				if(!WinUSBX[sub_api].AbortPipe(winusb_handle,
				...
				if(!WinUSBX[sub_api].ResetPipe(winusb_handle,
				                               priv->usb_interface[i].endpoint[j]))
					usbi_err(ctx,
                                      ...
                                      uint32_t              io_size)
{
	...
	if(transfer->type == LIBUSB_TRANSFER_TYPE_ISOCHRONOUS)
	{
		...
		// for isochronous, need to copy the individual iso packet actual_lengths and
		// statuses
		if((sub_api == SUB_API_LIBUSBK) || (sub_api == SUB_API_LIBUSB0))
		{
			...
			for(i = 0; i < transfer->num_iso_packets; i++)
			{
				transfer->iso_packet_desc[i].actual_length =
				    iso_context->IsoPackets[i].actual_length;
				// TODO translate USDB_STATUS codes
				// http://msdn.microsoft.com/en-us/library/ff539136(VS.85).aspx to
				// libusb_transfer_status
				// transfer->iso_packet_desc[i].status =
				// transfer_priv->iso_context->IsoPackets[i].status;
			}
		}
		else
		{
			// This should only occur if backend is not set correctly or other backend
			// isoc is partially implemented
...
static int _hid_get_report_descriptor(struct hid_device_priv* dev,
                                      void*                   data,
                                      size_t*                 size);
...
static int _hid_get_device_descriptor(struct hid_device_priv* dev,
                                      void*                   data,
                                      size_t*                 size)
{
...
static int _hid_get_config_descriptor(struct hid_device_priv* dev,
                                      void*                   data,
                                      size_t*                 size)
{
...
static int _hid_get_string_descriptor(
    struct hid_device_priv* dev, int _index, void* data, size_t* size, HANDLE hid_handle)
{
	void*  tmp = NULL;
...
static int _hid_get_report(struct hid_device_priv*      dev,
                           ...
                           int                          id,
                           void*                        data,
                           struct winusb_transfer_priv* tp,
                           size_t*                      size,
                           OVERLAPPED*                  overlapped,
                           int                          report_type)
{
	...
	// NB: The size returned by DeviceIoControl doesn't include report IDs when not in use
	// (0)
	                    ...
	                    overlapped))
	{
		...
		tp->hid_buffer = buf;
		tp->hid_dest =
		    data;  // copy dest, as not necessarily the start of the transfer buffer
		return LIBUSB_SUCCESS;
	}
	...
	{
		usbi_warn(NULL,
		          "program assertion failed - read completed synchronously, but no data "
		          "was read");
		*size = 0;
	}
	else
	{
		if(buf[0] != id)
			usbi_warn(NULL,
			          "mismatched report ID (data is %02X, parameter is %02X)",
			          buf[0],
			          id);
                           ...
                           int                          report_type)
{
	...
	*size = write_size;
	if(write_size == 0)
		usbi_dbg(
		    "program assertion failed - write completed synchronously, but no data was "
		    "written");
                              ...
                              OVERLAPPED*                  overlapped)
{
	...
	if(LIBUSB_REQ_OUT(request_type) && request == HID_REQ_SET_REPORT)
		return _hid_set_report(
		    dev, hid_handle, report_id, data, tp, size, overlapped, report_type);

	if(LIBUSB_REQ_IN(request_type) && request == HID_REQ_GET_REPORT)
		return _hid_get_report(
		    dev, hid_handle, report_id, data, tp, size, overlapped, report_type);
...
static void hid_exit(void) { DLL_FREE_HANDLE(hid); }
	...
	for(i = 0; i < USB_MAXINTERFACES; i++)
	{
		   ...
		   (priv->usb_interface[i].apib->id == USB_API_HID))
		{
			hid_handle = CreateFileA(priv->usb_interface[i].path,
			                         ...
			                         NULL);
			/*
			 * http://www.lvr.com/hidfaq.htm: Why do I receive "Access denied" when
			 * attempting to access my HID? "Windows 2000 and later have exclusive
			 * read/write access to HIDs that are configured as a system keyboards or
			 * mice. An application can obtain a handle to a system keyboard or mouse by
			 * not requesting READ or WRITE access with CreateFile. Applications can then
			 * use HidD_SetFeature and HidD_GetFeature (if the device supports Feature
			 * reports)."
			 */
			if(hid_handle == INVALID_HANDLE_VALUE)
			{
				usbi_warn(ctx,
				          "could not open HID device in R/W mode (keyboard or mouse?) - "
				          "trying without");
				hid_handle = CreateFileA(priv->usb_interface[i].path,
				                         0,
				                         FILE_SHARE_WRITE | FILE_SHARE_READ,
	...
	do
	{
		...
		for(j = HidP_Input; j <= HidP_Feature; j++)
		{
			...
			if(size[j] > 0)
			{
				...
				if((value_caps != NULL) &&
				   (HidP_GetValueCaps(
				        (HIDP_REPORT_TYPE)j, value_caps, &size[j], preparsed_data) ==
				    HIDP_STATUS_SUCCESS) &&
				   (size[j] >= 1))
				{
					nb_ids[0] = 0;
					...
					if(nb_ids[1] != 0)
					{
						if(nb_ids[0] != 0)
							usbi_warn(ctx,
							          "program assertion failed: zero and nonzero report "
							          "IDs used for %s",
							          type[j]);
		...
		if(priv->hid->string_index[1] != 0)
			HidD_GetProductString(
			    hid_handle, priv->hid->string[1], sizeof(priv->hid->string[1]));
		else
...
static int hid_submit_control_transfer(int sub_api, struct usbi_transfer* itransfer)
{
	...
	case LIBUSB_REQUEST_TYPE_STANDARD:
		...
		case LIBUSB_REQUEST_GET_DESCRIPTOR:
			r = _hid_get_descriptor(priv->hid,
			                        hid_handle,
			                        LIBUSB_REQ_RECIPIENT(setup->RequestType),
			                        (setup->Value >> 8) & 0xFF,
			                        setup->Value & 0xFF,
			                        transfer->buffer + LIBUSB_CONTROL_SETUP_SIZE,
			                        &size);
		...
		case LIBUSB_REQUEST_SET_INTERFACE:
			r = hid_set_interface_altsetting(
			    0, transfer->dev_handle, setup->Index, setup->Value);
			if(r == LIBUSB_SUCCESS)
				r = LIBUSB_COMPLETED;
			break;
		default:
	...
	{
		// Force request to be completed synchronously. Transferred size has been set by
		// previous call
...
static int hid_submit_bulk_transfer(int sub_api, struct usbi_transfer* itransfer)
{
	...
	transfer_priv->handle = hid_handle =
	    handle_priv->interface_handle[current_interface].api_handle;
	overlapped   = transfer_priv->pollable_fd.overlapped;
	...
	if(direction_in)
	{
		...
		ret = ReadFile(
		    hid_handle, transfer_priv->hid_buffer, length + 1, &size, overlapped);
	}
	else
	{
		...
		usbi_dbg(
		    "writing %d bytes (report ID: 0x%02X)", length, transfer_priv->hid_buffer[0]);
		...
		{  // Data readout
			if(corrected_size > 0)
			{
				...
				if(transfer_priv->hid_buffer[0] == 0)
				{
					...
					corrected_size--;
					memcpy(transfer_priv->hid_dest,
					       transfer_priv->hid_buffer + 1,
					       corrected_size);
				}
				else
				{
					memcpy(transfer_priv->hid_dest,
					       transfer_priv->hid_buffer,
					       corrected_size);
				}
			}
			transfer_priv->hid_dest = NULL;
	...
	for(pass = 0; pass < 2; pass++)
	{
		for(iface = 0; iface < USB_MAXINTERFACES; iface++)
		{
			   ...
			   (priv->usb_interface[iface].apib->submit_control_transfer != NULL))
			{
				...
				{
					usbi_dbg(
					    "trying to skip restricted interface #%d (HID keyboard or "
					    "mouse?)",
					    iface);
					continue;
				}
				...
				r = priv->usb_interface[iface].apib->submit_control_transfer(
				    priv->usb_interface[iface].sub_api, itransfer);
				// If not supported on this API, it may be supported on another, so don't
				// give up yet!!
...
static int composite_abort_control(int sub_api, struct usbi_transfer* itransfer)
{
	...
	return priv->usb_interface[current_interface].apib->abort_control(
	    priv->usb_interface[current_interface].sub_api, itransfer);
}
...
static int composite_abort_transfers(int sub_api, struct usbi_transfer* itransfer)
{
	...
	return priv->usb_interface[current_interface].apib->abort_transfers(
	    priv->usb_interface[current_interface].sub_api, itransfer);
}
                                        ...
                                        uint32_t              io_size)
{
	...
	return priv->usb_interface[current_interface].apib->copy_transfer_data(
	    priv->usb_interface[current_interface].sub_api, itransfer, io_size);
}
