		...
		if(fDevice != NULL && fDevice->InitCheck() == true)
		{
			...
			list_for_each_entry(ctx, &active_contexts_list, list, struct libusb_context)
			{
				struct libusb_device* dev =
				    usbi_get_device_by_session_id(ctx, session_id);
				if(dev)
				{
					usbi_dbg("using previously allocated device with location %lu",
					         session_id);
...
bool WatchedEntry::InitCheck() { return fInitCheck; }
...
bool RosterLooper::InitCheck() { return fInitCheck; }

USBRoster::USBRoster() : fLooper(NULL) {}

USBRoster::~USBRoster() { Stop(); }
