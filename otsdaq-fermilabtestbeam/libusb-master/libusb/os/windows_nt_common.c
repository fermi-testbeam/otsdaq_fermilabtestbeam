...
DLL_DECLARE_FUNC_PREFIXED(
    WINAPI, BOOL, p, PostThreadMessageA, (DWORD, UINT, WPARAM, LPARAM));
...
const char* windows_error_str(DWORD error_code)
{
	...
	case 0:
		error_code =
		    HRESULT_FROM_WIN32(error_code);  // Still leaves ERROR_SUCCESS unmodified
		break;
	case 0xE0000000:
...
static BOOL windows_init_dlls(void)
{
	...
	pCancelIoEx = (BOOL(WINAPI*)(HANDLE, LPOVERLAPPED))GetProcAddress(
	    DLL_HANDLE_NAME(Kernel32), "CancelIoEx");
...
static void get_windows_version(void)
{
	...
	if(vi.wServicePackMinor)
		usbi_dbg(
		    "Windows %s SP%u.%u %s", w, vi.wServicePackMajor, vi.wServicePackMinor, arch);
...
static unsigned __stdcall windows_clock_gettime_threaded(void* param)
{
	...
	while(1)
	{
		...
		{
			usbi_err(
			    NULL, "GetMessage failed for timer thread: %s", windows_error_str(0));
			return 1;
		}
		...
		case WM_TIMER_REQUEST:
			...
			request->tp->tv_nsec =
			    (long)(((hires_counter.QuadPart % hires_frequency) / 1000) *
			           hires_ticks_to_ps);
			if(!SetEvent(request->event))
				usbi_err(
				    NULL, "SetEvent failed for timer request: %s", windows_error_str(0));
			break;
		case WM_TIMER_EXIT:
                                      ...
                                      DWORD                         io_size)
{
	...
	usbi_dbg("handling I/O completion with errcode %u, size %u",
...
static int windows_init(struct libusb_context* ctx)
{
	...
	sprintf(
	    sem_name, "libusb_init%08X", (unsigned int)(GetCurrentProcessId() & 0xFFFFFFFF));
                                                  ...
                                                  int*            host_endian)
{
	...
	*host_endian                      = 0;
	return priv->backend->get_config_descriptor_by_value(
	    dev, bConfigurationValue, buffer);
}
...
static int windows_claim_interface(struct libusb_device_handle* dev_handle,
...
static int windows_release_interface(struct libusb_device_handle* dev_handle,
                                     int                          interface_number)
{
	struct windows_context_priv* priv = _context_priv(HANDLE_CTX(dev_handle));
...
{
	struct windows_context_priv* priv = _context_priv(HANDLE_CTX(dev_handle));
	return priv->backend->set_interface_altsetting(
	    dev_handle, interface_number, altsetting);
}

static int windows_clear_halt(struct libusb_device_handle* dev_handle,
	...
	case USBI_CLOCK_MONOTONIC:
		if(timer_thread)
		{
			...
			if(!pPostThreadMessageA(
			       timer_thread_id, WM_TIMER_REQUEST, 0, (LPARAM)&request))
			{
				usbi_err(NULL,
				         "PostThreadMessage failed for timer thread: %s",
				         windows_error_str(0));
			...
			do
			{
				...
				if(r == WAIT_TIMEOUT)
					usbi_dbg(
					    "could not obtain a timer value within reasonable timeframe - "
					    "too much load?");
				else if(r == WAIT_FAILED)
					usbi_err(
					    NULL, "WaitForSingleObject failed: %s", windows_error_str(0));
