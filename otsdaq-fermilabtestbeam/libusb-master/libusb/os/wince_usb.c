...
static int wince_get_device_list(struct libusb_context*   ctx,
...
err_out:
	*discdevs = new_devices;
	...
	UkwReleaseDeviceList(
	    driver_handle, &devices[release_list_offset], count - release_list_offset);
	return r;
}
...
static int wince_get_device_descriptor(struct libusb_device* device,
                                              ...
                                              int*                  host_endian)
{
	...
	*host_endian = 0;
	if(!UkwGetConfigDescriptor(
	       priv->dev, UKW_ACTIVE_CONFIGURATION, buffer, len, &actualSize))
...
static int wince_claim_interface(struct libusb_device_handle* handle,
...
static int wince_release_interface(struct libusb_device_handle* handle,
                                   int                          interface_number)
{
	struct wince_device_priv* priv = _device_priv(handle->dev);
...
static int wince_kernel_driver_active(struct libusb_device_handle* handle,
                                      int                          interface_number)
{
	struct wince_device_priv* priv   = _device_priv(handle->dev);
	BOOL                      result = FALSE;
...
static int wince_detach_kernel_driver(struct libusb_device_handle* handle,
                                      int                          interface_number)
{
	struct wince_device_priv* priv = _device_priv(handle->dev);
...
static int wince_attach_kernel_driver(struct libusb_device_handle* handle,
                                      int                          interface_number)
{
	struct wince_device_priv* priv = _device_priv(handle->dev);
...
static int wince_cancel_transfer(struct usbi_transfer* itransfer)
{
	...
	if(!UkwCancelTransfer(
	       priv->dev, transfer_priv->pollable_fd.overlapped, UKW_TF_NO_WAIT))
		return translate_driver_error(GetLastError());
...
static int wince_submit_control_or_bulk_transfer(struct usbi_transfer* itransfer)
{
	...
	if(control_transfer)
	{
		...
		ret = UkwIssueControlTransfer(priv->dev,
		                              flags,
		                              setup,
		                              buf,
		                              bufLen,
                                    ...
                                    uint32_t              io_size)
{
	...
	{
		/* For functional stalls, the WinCE USB layer (and therefore the USB Kernel
		 * Wrapper Driver) will report USB_ERROR_STALL/ERROR_NOT_SUPPORTED in situations
		 * where the endpoint isn't actually stalled.
		 *
		 * One example of this is that some devices will occasionally fail to reply to an
		 * IN token. The WinCE USB layer carries on with the transaction until it is
		 * completed (or cancelled) but then completes it with USB_ERROR_STALL.
		 *
		 * This code therefore needs to confirm that there really is a stall error, by
		 * both checking the pipe status and requesting the endpoint status from the
		 * device.
		 */
		BOOL halted = FALSE;
		usbi_dbg(
		    "checking I/O completion with errcode ERROR_NOT_SUPPORTED is really a stall");
		if(UkwIsPipeHalted(priv->dev, transfer->endpoint, &halted))
		{
			/* Pipe status retrieved, so now request endpoint status by sending a
			 * GET_STATUS control request to the device. This is done synchronously, which
			 * is a bit naughty, but this is a special corner case.
			 */
			WORD               wStatus = 0;
			DWORD              written = 0;
			UKW_CONTROL_HEADER ctrlHeader;
			ctrlHeader.bmRequestType = LIBUSB_REQUEST_TYPE_STANDARD | LIBUSB_ENDPOINT_IN |
			                           ...
			                           NULL))
			{
				if(written == sizeof(wStatus) && (wStatus & STATUS_HALT_FLAG) == 0)
				{
					...
					{
						usbi_dbg(
						    "Endpoint doesn't appear to be stalled, overriding error "
						    "with success");
					...
					{
						usbi_dbg(
						    "Endpoint doesn't appear to be stalled, but the host is "
						    "halted, changing error");
...
static void wince_handle_callback(struct usbi_transfer* itransfer,
                                  uint32_t              io_result,
                                  uint32_t              io_size)
{
	struct libusb_transfer* transfer = USBI_TRANSFER_TO_LIBUSB_TRANSFER(itransfer);
...
static int wince_clock_gettime(int clk_id, struct timespec* tp)
{
	...
	case USBI_CLOCK_MONOTONIC:
		if(hires_frequency != 0 && QueryPerformanceCounter(&hires_counter))
		{
			...
			tp->tv_nsec = (long)(((hires_counter.QuadPart % hires_frequency) / 1000) *
			                     hires_ticks_to_ps);
			return LIBUSB_SUCCESS;
		}
		// Fall through and return real-time if monotonic read failed or was not detected
		// @ init
