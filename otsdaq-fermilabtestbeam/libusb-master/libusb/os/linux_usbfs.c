...
static int sysfs_get_active_config(struct libusb_device* dev, int* config)
{
	...
	{
		usbi_err(
		    DEVICE_CTX(dev), "read bConfigurationValue failed ret=%d errno=%d", r, errno);
		return LIBUSB_ERROR_IO;
	}
	else if(r == 0)
	{
                               ...
                               int                    size)
{
	...
	{
		usbi_err(
		    ctx, "descriptor is not a config desc (type 0x%02x)", config.bDescriptorType);
		return LIBUSB_ERROR_IO;
	}
	...
	if(fd < 0)
	{
		...
		if(priv->descriptors_len >= (DEVICE_DESC_LENGTH + LIBUSB_DT_CONFIG_SIZE))
		{
			...
			usbi_parse_descriptor(
			    priv->descriptors + DEVICE_DESC_LENGTH, "bbwbbbbb", &config, 0);
                           ...
                           const char*            sysfs_dir)
{
	...
	usbi_dbg(
	    "allocating new device for %d/%d (session %ld)", busnum, devaddr, session_id);
...
static int claim_interface(struct libusb_device_handle* handle, int iface)
{
	...
	if(r)
	{
		...
		usbi_err(
		    HANDLE_CTX(handle), "claim interface failed, error %d errno %d", r, errno);
		return LIBUSB_ERROR_OTHER;
	}
	return 0;
}
...
static int release_interface(struct libusb_device_handle* handle, int iface)
{
	...
	if(r)
	{
		...
		usbi_err(
		    HANDLE_CTX(handle), "release interface failed, error %d errno %d", r, errno);
		return LIBUSB_ERROR_OTHER;
	}
	return 0;
}
...
{
	return do_streams_ioctl(
	    handle, IOCTL_USBFS_FREE_STREAMS, 0, endpoints, num_endpoints);
}
		...
		if(r < 0)
		{
			...
			else
			{
				usbi_err(TRANSFER_CTX(transfer),
				         "submiturb failed error %d errno=%d",
				         r,
				         errno);
				r = LIBUSB_ERROR_IO;
			}
...
static int submit_iso_transfer(struct usbi_transfer* itransfer)
{
	...
	for(i = 0, j = 0; i < num_urbs; i++)
	{
		...
		alloc_size =
		    sizeof(*urb) + (num_packets_in_urb * sizeof(struct usbfs_iso_packet_desc));
...
static int handle_bulk_completion(struct usbi_transfer* itransfer, struct usbfs_urb* urb)
{
	...
	{
		usbi_dbg(
		    "short transfer %d/%d --> complete!", urb->actual_length, urb->buffer_length);
		if(tpriv->reap_action == NORMAL)
...
static int handle_iso_completion(struct usbi_transfer* itransfer, struct usbfs_urb* urb)
{
	...
	usbi_dbg(
	    "handling completion status %d of iso urb %d/%d", urb->status, urb_idx, num_urbs);
	...
	for(i = 0; i < urb->number_of_packets; i++)
	{
		...
		default:
			usbi_warn(
			    TRANSFER_CTX(transfer), "unrecognised urb status %d", urb_desc->status);
			lib_desc->status = LIBUSB_TRANSFER_ERROR;
			break;
		}
