...
int actual_length;
int r = libusb_bulk_transfer(dev_handle, LIBUSB_ENDPOINT_IN, data, sizeof(data),
&actual_length, 0); if (r == 0 && actual_length == sizeof(data)) {
...
#ifdef USBI_TIMERFD_AVAILABLE
	ctx->timerfd =
	    timerfd_create(usbi_backend.get_timerfd_clockid(), TFD_NONBLOCK | TFD_CLOEXEC);
	if(ctx->timerfd >= 0)
	{
...
static int calculate_timeout(struct usbi_transfer* transfer)
{
	...
	{
		usbi_err(
		    ITRANSFER_CTX(transfer), "failed to read monotonic clock, errno=%d", errno);
		return r;
	}
...
static int remove_from_flying_list(struct usbi_transfer* transfer)
{
	...
	rearm_timerfd = (timerisset(&transfer->timeout) &&
	                 list_first_entry(
	                     &ctx->flying_transfers, struct usbi_transfer, list) == transfer);
	list_del(&transfer->list);
                                    ...
                                    enum libusb_transfer_status status)
{
	...
	if(r < 0)
		usbi_err(ITRANSFER_CTX(itransfer),
		         "failed to set timer for next timeout, errno=%d",
		         errno);
...
static int handle_events(struct libusb_context* ctx, struct timeval* tv)
{
	...
	if(fds[0].revents)
	{
		...
		{
			itransfer = list_first_entry(
			    &ctx->completed_transfers, struct usbi_transfer, completed_list);
			list_del(&itransfer->completed_list);
			usbi_mutex_unlock(&ctx->event_data_lock);
			ret = usbi_backend.handle_transfer_completion(itransfer);
			if(ret)
				usbi_err(
				    ctx, "backend handle_transfer_completion failed with error %d", ret);
			usbi_mutex_lock(&ctx->event_data_lock);
		}
...
void usbi_remove_pollfd(struct libusb_context* ctx, int fd)
{
	...
	usbi_mutex_lock(&ctx->event_data_lock);
	list_for_each_entry(
	    ipollfd, &ctx->ipollfds, list, struct usbi_pollfd) if(ipollfd->pollfd.fd == fd)
	{
		found = 1;
		break;
	}
...
void usbi_handle_disconnect(struct libusb_device_handle* dev_handle)
{
	...
	usbi_dbg(
	    "device %d.%d", dev_handle->dev->bus_number, dev_handle->dev->device_address);
