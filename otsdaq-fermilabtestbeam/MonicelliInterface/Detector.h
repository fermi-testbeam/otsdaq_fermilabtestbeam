...
class Detector : public TObject
{
  public:
	...
	double      getAlignmentPredictedGlobal(ROOT::Math::SVector<double, 4>& trackPars,
	                                        matrix33Def&                    RInv,
	                                        double                          z,
	                                        double&                         predX,
	                                        double&                         predY);
	void        getPredictedGlobal(ROOT::Math::SVector<double, 4>& trackPars,
	                               double&                         predX,
	                               double&                         predY,
	                               double&                         predZ);
	void        getPredictedLocal(ROOT::Math::SVector<double, 4>& trackPars,
	                              double&                         predX,
	                              double&                         predY);
	xyPair      getTrackErrorsOnPlane(ROOT::Math::SVector<double, 4>& trackPars,
	                                  matrix44Def&                    AtVAInv);
	xyPair      propagateTrackErrors(ROOT::Math::SVector<double, 4>& trackPars,
	                                 matrix44Def&                    AtVAInv,
	                                 Detector::matrix33Def&          RInv,
	                                 double                          z);
	...
	{
		xRotationCorrectionError_ = xRotCorErr;
	}
	...
	{
		yRotationCorrectionError_ = yRotCorErr;
	}
	...
	{
		zRotationCorrectionError_ = zRotCorErr;
	}
  ...
  private:
	...
	void XRotation(
	    double* y, double* z, double* yErr, double* zErr, bool backward = false);
	void YRotation(
	    double* x, double* z, double* xErr, double* zErr, bool backward = false);
	void ZRotation(
	    double* x, double* y, double* xErr, double* yErr, bool backward = false);
	...
	// std::stringstream      ss_                                     ;//! temporary state
	// value
