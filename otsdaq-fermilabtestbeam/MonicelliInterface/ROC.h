...
class ROC : public TObject
{
  public:
	...
	//   void         setCalibrationFunction    (int row, int col, double *par, double
	//   *cov, int nPars) ;
	...
	{
		xLocalPosition_ = xLocalPosition;
	}
	void setLocalYPosition(unsigned int yLocalPosition)
	{
		yLocalPosition_ = yLocalPosition;
	}
  ...
  private:
	typedef std::map<int, std::map<int, ROOT::Math::SMatrix<double, 4> > >
	    pixelCalibrationsCovDef;
	// row picth   , col pitch
	...
	typedef std::map<int, std::map<int, std::vector<double> > >
	    pixelCalibrationsTmpMapDef;
	...
	//   pixelCalibrationsMapDef       pixelCalibrationFunction_            ;//! temporary
	//   state value
