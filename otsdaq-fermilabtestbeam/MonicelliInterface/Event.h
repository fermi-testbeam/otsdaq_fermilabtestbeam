...
**
**
**  This is the header of the event class. An event is basically a collection of the
*following
**  quantities:
**
**  1] raw data hits (identified by row/column on a plaquette in the telescope)
**  2] the corresponding cluster coordinates (obtained by collapsing clusters of adjacent
*raw
**     data hits into a single x/y pair of coordinates by appliying a charge sharing
*algorithm)
**  3] fitted tracks (obtained by fitting strings of aligned cluster-hits to a straight
*line in
...
**
**  Below is a description of the basic data structures which are made persistent to disk
*by
...
**
*----------------------------------------------------------------------------------------------
**  The cluster hit coordinates/errors computed for each plaquette by using the charge
*sharing
...
**
*----------------------------------------------------------------------------------------------
**  The vector of fitted tracks: for each plaquette the coordinates/errors of the cluster
*hits
**  used for the final fit are provided in the laboratory reference frame. Additionally it
*is
**  possible to gain access to the cluster used to perform the fit by means of the
*"cluster ID"
**
**  vector<map<string, map<string, double> > >                            trackCandidates_
**    |          |           |       |
**    |          |           |       +--------------> value
**    |          |           +----------------------> x | y | z | xErr | yErr | errZ |
*size | cluster ID
**    |          +----------------------------------> plaquette  ID
...
**
*----------------------------------------------------------------------------------------------
**  The unconstrained residuals of the fitted tracks on the two projections. This vector
*is
...
**
*----------------------------------------------------------------------------------------------
**  The vector of the slope/intercept parameters of the fitted tracks. This vector is
*strictly
**  parallel to fittedTrackResiduals_.
**
**  vector<SVector<double,4> >                                            fittedTracks_
**     |             |
**     |             +------------------------------> slope/intercept on x and y
*projections
...
**
*----------------------------------------------------------------------------------------------
**  The vector of the slope/intercept parameters of the unconstrained fitted tracks. This
*vector is strictly
...
**
*----------------------------------------------------------------------------------------------
**  The vector of the covariance matrix of the unconstrained fitted tracks. This vector is
*strictly
...
class Event : public TObject
{
  public:
	...
	typedef std::vector<std::map<std::string, matrixDef> >
	...
	//    void                       setTrackCandidates        (const trackCandidatesDef&
	//    trackCandidates ){trackCandidates_        = trackCandidates  ;}
	void setFittedTrackResiduals(const residualsMapDef& residuals)
	{
		fittedTrackResiduals_ = residuals;
	}
	void setFittedTrackDeviations(const residualsMapDef& residuals)
	{
		fittedTrackDeviations_ = residuals;
	}
