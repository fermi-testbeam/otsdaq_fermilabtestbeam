...
#include "otsdaq-fermilabtestbeam/MonicelliInterface/ROC.h"
...
void ROC::setRowPitchVector(void)
{
	...
	for(nonStandardPitchMapDef::iterator it = nonStandardRowPitch_.begin();
	    it != nonStandardRowPitch_.end();
	    it++)
	{
	...
	rocLengthY_ =
	    rowLowEdge_[rowLowEdge_.size() - 1] + rowPitches_[rowPitches_.size() - 1];
}
...
void ROC::setColPitchVector(void)
{
	...
	for(nonStandardPitchMapDef::iterator it = nonStandardColPitch_.begin();
	    it != nonStandardColPitch_.end();
	    it++)
	{
	...
	rocLengthX_ =
	    colLowEdge_[colLowEdge_.size() - 1] + colPitches_[colPitches_.size() - 1];
}
...
double ROC::getPixelCenterLocalX(unsigned int pixelCol)
{
	if(!goodCol(pixelCol))
		return -1;
	return (this->getPixelLowEdgeLocalX(pixelCol) +
	        this->getPixelPitchLocalX(pixelCol) / 2.);
}
...
double ROC::getPixelCenterLocalY(unsigned int pixelRow)
{
	if(!goodRow(pixelRow))
		return -1;
	return (this->getPixelLowEdgeLocalY(pixelRow) +
	        this->getPixelPitchLocalY(pixelRow) / 2.);
}
...
//==========================================================================
double ROC::getLengthLocalX(void) { return rocLengthX_; }

//==========================================================================
double ROC::getLengthLocalY(void) { return rocLengthY_; }
...
{
	if(pixelCalibrationFunctionTmp_.count(row) &&
	   pixelCalibrationFunctionTmp_[row].count(col))
		return true;
	else
		return false;
}
...
//============================================================================
void ROC::setCalibrationFunction(int     row,
                                 int     col,
                                 double* par,
                                 double* cov)  //, int nPars )
{
	if(pixelCalibrationFunctionTmp_.count(row) &&
	   pixelCalibrationFunctionTmp_[row].count(col))
...
double ROC::getCalibrationError(int /*row*/, int /*col*/, int /*adc*/)
{
	...
	pixelCalibrationFunctionTmp_[row].count(col) )
	{
	    double arg  = ( adc - pixelCalibrationFunctionTmp_[row][col][0]
	)/pixelCalibrationFunctionTmp_[row][col][1]; double der0 = -1./(
	pixelCalibrationFunctionTmp_[row][col][2]*pow(cosh(arg),2) ); double der1 = -arg/(
	pow(cosh(arg),2)*pixelCalibrationFunctionTmp_[row][col][2]*pixelCalibrationFunctionTmp_[row][col][1]
	); double der2 = ( pixelCalibrationFunctionTmp_[row][col][3] - tanh(arg) ) / (
	pixelCalibrationFunctionTmp_[row][col][2]*pixelCalibrationFunctionTmp_[row][col][2] );
	    ...
	    ss_.str("");
	    ss_ << "der0: " << der0 << " der1: " << der1 << " der2: " << der2 << " der3: " <<
	der3;
	    //STDLINE(ss_.str(), ACPurple);
	    ss_.str("");
	    ss_ << "eer0: " << pixelCalibrationCovMat_[row][col][0][0] << " eer1: " <<
	pixelCalibrationCovMat_[row][col][1][1]
	        << " eer2: " << pixelCalibrationCovMat_[row][col][2][2] << " eer3: " <<
	pixelCalibrationCovMat_[row][col][3][3];
