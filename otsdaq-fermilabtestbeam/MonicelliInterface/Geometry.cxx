...
#include "otsdaq-fermilabtestbeam/MonicelliInterface/MessageTools.h"
...
//=============================================================================
void Geometry::setGeometryFileName(std::string fileName) { geometryFileName_ = fileName; }

//=============================================================================
bool Geometry::compare_zPosition(std::string first, std::string second)
{
	if(this->getDetector(first)->getZPositionTotal() <
	   this->getDetector(second)->getZPositionTotal())
		return true;
	else
		return false;
}
...
Detector* Geometry::addDetector(std::string plaqID, bool isDUT)
{
	if(detectorsMap_.find(plaqID) == detectorsMap_.end())
	{
		...
		STDLINE("Warning: Detector: " + plaqID + " was already added!!", ACRed);
...
unsigned int Geometry::getMaxRowsNum(void)
{
	...
	for(detectorsMapDef::iterator it = detectorsMap_.begin(); it != detectorsMap_.end();
	    ++it)
	{
		if((*it).second->getNumberOfRows() > maxRowsNum)
			maxRowsNum = (*it).second->getNumberOfRows();
	}
	return maxRowsNum;
}
...
unsigned int Geometry::getMaxColsNum(void)
{
	...
	for(detectorsMapDef::iterator it = detectorsMap_.begin(); it != detectorsMap_.end();
	    ++it)
	{
		if((*it).second->getNumberOfCols() > maxColsNum)
			maxColsNum = (*it).second->getNumberOfCols();
	}
	return maxColsNum;
}
...
double Geometry::getMaxDetectorsLength(void)
{
	...
	for(detectorsMapDef::iterator it = detectorsMap_.begin(); it != detectorsMap_.end();
	    ++it)
	{
		if((*it).second->getDetectorLengthX() > maxDetectorsLength)
			maxDetectorsLength = (*it).second->getDetectorLengthX();
		if((*it).second->getDetectorLengthY() > maxDetectorsLength)
			maxDetectorsLength = (*it).second->getDetectorLengthY();
	}
...
unsigned int Geometry::getDetectorsNumber(bool excludeDUT)
{
	if(!excludeDUT)
		...
		return (detectorsMap_.size() - dutNumbers_);
}
