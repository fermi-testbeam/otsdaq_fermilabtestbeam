...
Detector::~Detector(void)
{
	...
	for(ROCsMapDef::iterator it = ROCsChipIDMap_.begin(); it != ROCsChipIDMap_.end();
	    it++)
	{
		delete it->second;
	}
...
void Detector::setupVariables(void)
{
	    ...
	    it++)
	{
		...
		unsigned int yPosition =
		    ROCPosition / xNumberOfROCs_;  // division between ints is what i want!

		it->second->setLocalYPosition(yPosition);
		if(yPosition % 2 != 0)
			it->second->setLocalXPosition((yPosition + 1) * xNumberOfROCs_ -
			                              (ROCPosition + 1));
		else
			it->second->setLocalXPosition(ROCPosition - yPosition * xNumberOfROCs_);
	...
	{
		ROCsPositionMap_[posX]->setFirstCol(ROCsPositionMap_[(posX - 1)]->getLastCol() +
		                                    1);
	...
	for(unsigned int posY = xNumberOfROCs_; posY < yNumberOfROCs_ * xNumberOfROCs_;
	    posY += xNumberOfROCs_)
	{
		ROCsPositionMap_[posY]->setFirstRow(
		    ROCsPositionMap_[(posY - xNumberOfROCs_)]->getLastRow() + 1);
	...
	for(ROCsMapDef::iterator it = ROCsChipIDMap_.begin(); it != ROCsChipIDMap_.end();
	    it++)
	{
		it->second->setFirstCol(
		    ROCsPositionMap_[it->second->getPositionLocalX()]->getFirstCol());
		it->second->setLastCol(
		    ROCsPositionMap_[it->second->getPositionLocalX()]->getLastCol());
		it->second->setFirstRow(
		    ROCsPositionMap_[it->second->getPositionLocalY() * xNumberOfROCs_]
		        ->getFirstRow());
		it->second->setLastRow(
		    ROCsPositionMap_[it->second->getPositionLocalY() * xNumberOfROCs_]
		        ->getLastRow());
	}
}
...
ROC* Detector::addROC(unsigned int chipPosition, int chipID, unsigned int rotationDegrees)
{
	if(chipPosition > numberOfROCs_)
	{
		...
		ss.str("");
		ss << chipID;
		STDLINE("ERROR: there is no ROC position: " + ss.str() + " in current detector!!",
		        ACRed);
		STDLINE("ERROR: Maybe you have to use setComposition first", ACRed);
	}
	if(ROCsChipIDMap_.find(chipID) == ROCsChipIDMap_.end() &&
	   ROCsPositionMap_.find(chipID) == ROCsPositionMap_.end())
	{
	...
	else
	{
		std::stringstream ss;
		ss.str("");
		ss << "WARNING: ROC: " << chipID << " in position " << chipPosition
		   << "was already set!!";
...
unsigned int Detector::getNumberOfCols(bool global)
{
	if(!global || (zRotation_ < 45 && zRotation_ >= 0) ||
	   (zRotation_ <= 360 && zRotation_ > 315) || (zRotation_ > 135 && zRotation_ < 225))
...
double Detector::getPixelLowEdgeLocalX(unsigned int col)
{
	...
	STDLINE("WARNING: pixel column is out of range, returning detector's length X",
	        ACRed);
	return lowEdge;
}
...
void Detector::convertPixelFromROC(ROC* roc, unsigned int* row, unsigned int* col)
{
	...
	{
		STDLINE("ERROR: Only 0 and 180 degrees rotations are supported for a ROC!",
		        ACRed);
		exit(EXIT_FAILURE);
	}
}
...
ROC* Detector::findROC(unsigned int row, unsigned int col)
{
	...
	STDLINE("WARNING: pixel column or pixel row is out of detector's range, return NULL",
	        ACRed);
	return 0;
}
...
{
	// vertical clockwise
	if((zRotation_ > 225 && zRotation_ < 315) || (zRotation_ > 45 && zRotation_ < 135))
		return true;
	return false;
}
//=================================================================
...
//=======================================================================================
double Detector::fromLocalToGlobal(
    double* x, double* y, double* z, double* xErr, double* yErr, double* zErr)
{
...
//=======================================================================================
void Detector::fromLocalToGlobalNoRotation(double* x,
                                           double* y,
                                           double* xErr,
                                           double* yErr)
{
...
//=======================================================================================
void Detector::fromGlobalToLocal(
    double* x, double* y, double* z, double* xErr, double* yErr, double* zErr)
{
	*z -= this->getZPositionTotal();
...
//=======================================================================================
void Detector::XYZRotation(double* x,
                           double* y,
                           double* z,
                           double* xErr,
                           double* yErr,
                                             ...
                                             double&                         predY)
{
	...
	den = (RInv[0][0] - trackPars[0] * RInv[2][0]) *
	          (RInv[1][1] - trackPars[2] * RInv[2][1]) -
	      (RInv[1][0] - trackPars[2] * RInv[2][0]) *
	          (RInv[0][1] - trackPars[0] * RInv[2][1]);
	predX = numX / den;
	predY = numY / den;
	return den;
}
...
//================================================================
void Detector::getPredictedGlobal(ROOT::Math::SVector<double, 4>& trackPars,
                                  double&                         predX,
                                  double&                         predY,
                                  double&                         predZ)
{
	...
	Detector::matrix33Def R = this->getRotationMatrix();
	predZ =
	    (R(0, 2) * (x0 - trackPars[1]) + R(1, 2) * (y0 - trackPars[3]) + R(2, 2) * z0) /
	    (R(0, 2) * trackPars[0] + R(1, 2) * trackPars[2] + R(2, 2));
	...
	double N_X0 = (trackPars[0]*z0 + trackPars[1]) * (R(1,1) - trackPars[2]*R(2,1)) -
	(trackPars[2]*z0 + trackPars[3]) * (R(0,1) - trackPars[0]*R(2,1)); double N_Y0 =
	(trackPars[2]*z0 + trackPars[3]) * (R(0,0) - trackPars[0]*R(2,0)) - (trackPars[0]*z0 +
	trackPars[1]) * (R(1,0) - trackPars[2]*R(2,0)); double D_0 = (R(0,0) -
	trackPars[0]*R(2,0)) * (R(1,1) - trackPars[2]*R(2,1)) - (R(1,0) - trackPars[2]*R(2,0))
	* (R(0,1) - trackPars[0]*R(2,1)); double predXX = N_X0 / D_0; double predYY = N_Y0 /
	D_0;
	...
	N_X0 = (trackPars[0]*z0 + trackPars[1]) * (R(1,1) - trackPars[2]*R(2,1)) -
	(trackPars[2]*z0 + trackPars[3]) * (R(0,1) - trackPars[0]*R(2,1)); N_Y0 =
	(trackPars[2]*z0 + trackPars[3]) * (R(0,0) - trackPars[0]*R(2,0)) - (trackPars[0]*z0 +
	trackPars[1]) * (R(1,0) - trackPars[2]*R(2,0)); D_0 = (R(0,0) - trackPars[0]*R(2,0)) *
	(R(1,1) - trackPars[2]*R(2,1)) - (R(1,0) - trackPars[2]*R(2,0)) * (R(0,1) -
	trackPars[0]*R(2,1)); predXX = N_X0 / D_0; predYY = N_Y0 / D_0;
...
//================================================================
void Detector::getPredictedLocal(ROOT::Math::SVector<double, 4>& trackPars,
                                 double&                         predX,
                                 double&                         predY)
{
	double predZ = 0;
...
//======================================================================================================
Detector::xyPair Detector::getTrackErrorsOnPlane(
    ROOT::Math::SVector<double, 4>& trackPars, matrix44Def& AtVAInv)
{
                                                ...
                                                double                          z)
{
	...
	double dfNX_dslx = z * (RInv[1][1] - trackPars[2] * RInv[2][1]) -
	                   (trackPars[2] * z + trackPars[3]) * (-RInv[2][1]);
	double dfNX_dqx  = (RInv[1][1] - trackPars[2] * RInv[2][1]);
	double dfNX_dsly = (trackPars[0] * z + trackPars[1]) * (-RInv[2][1]) -
	                   (z) * (RInv[0][1] - trackPars[0] * RInv[2][1]);
	double dfNX_dqy  = -(RInv[0][1] - trackPars[0] * RInv[2][1]);
	double dfNY_dslx = (trackPars[2] * z + trackPars[3]) * (-RInv[2][0]) -
	                   (z + trackPars[1]) * (RInv[1][0] - trackPars[2] * RInv[2][0]);
	double dfNY_dqx  = -(RInv[1][0] - trackPars[2] * RInv[2][0]);
	double dfNY_dsly = (z) * (RInv[0][0] - trackPars[0] * RInv[2][0]) -
	                   (trackPars[0] * z + trackPars[1]) * (-RInv[2][0]);
	double dfNY_dqy = (RInv[0][0] - trackPars[0] * RInv[2][0]);
	double dfD_dslx = (-RInv[2][0]) * (RInv[1][1] - trackPars[2] * RInv[2][1]) -
	                  (RInv[1][0] - trackPars[2] * RInv[2][0]) * (-RInv[2][1]);
	// double dfD_dqx    = 0;
	double dfD_dsly = (RInv[0][0] - trackPars[0] * RInv[2][0]) * (-RInv[2][1]) -
	                  (-RInv[2][0]) * (RInv[0][1] - trackPars[0] * RInv[2][1]);
	// double dfD_dqy    = 0;
	...
	predSigmaXX = dpredx_dslx * dpredx_dslx * AtVAInv(0, 0) +
	...
	predSigmaYY = dpredy_dslx * dpredy_dslx * AtVAInv(0, 0) +
    ...
    double* x, double* y, double* z, double* xErr, double* yErr, double* zErr)
{
	...
	ss.str("");
	ss << "************************   " << ID_ << "
	**************************************"; STDLINE(ss.str(),ACYellow); ss.str(""); ss <<
	__PRETTY_FUNCTION__ << "After fromLocalToGlobal1->"
	        << " x: " << xc1 << " y: " << yc1 << " z:" << zc1
	...
	ss << "************************   " << ID_ << "
	**************************************"; STDLINE(ss.str(),ACYellow); ss.str(""); ss <<
	__PRETTY_FUNCTION__ << "After fromLocalToGlobal2->"
	        << " x: " << xc2 << " y: " << yc2 << " z:" << zc2
	...
	ss << "************************   " << ID_ << "
	**************************************"; STDLINE(ss.str(),ACYellow); ss.str(""); ss <<
	__PRETTY_FUNCTION__ << "After DeltaGlobal       ->"
	        << " x: " << xc1-xc2 << " y: " << yc1-yc2 << " z:" << zc1-zc2
	        << " xErr: " << xErrc1-xErrc2 << " yErr: " << yErrc1-yErrc2 << " zErr:" <<
	zErrc1-zErrc2; STDLINE(ss.str(),ACRed); ss.str(""); ss << __PRETTY_FUNCTION__ <<
	"After DeltaGlobal square->"
	        << " x: " << pow(xc1-xc2,2) << " y: " << pow(yc1-yc2,2) << " z:" <<
	pow(zc1-zc2,2)
	        << " Tot: " << sqrt(pow(xc1-xc2,2)+pow(yc1-yc2,2)+pow(zc1-zc2,2)) << " = " <<
	sqrt(2*pow(100,2)); STDLINE(ss.str(),ACRed);

	fromGlobalToDetector(&xc1,&yc1,&zc1,&xErrc1,&yErrc1,&zErrc1);
	ss.str("");
	ss << "************************   " << ID_ << "
	**************************************"; STDLINE(ss.str(),ACYellow); ss.str(""); ss <<
	__PRETTY_FUNCTION__ << "After fromGlobalToLocal1->"
	        << " x: " << xc1 << " y: " << yc1 << " z:" << zc1
	        << " xErr: " << xErrc1 << " yErr: " << yErrc1 << " zErr:" << zErrc1;
	STDLINE(ss.str(),ACGreen);
...
*/
	...
	ss.str("");
	ss << __PRETTY_FUNCTION__ << "Original Values->"
	   << " x: " << *x << " y: " << *y << " z:" << *z << " xErr: " << *xErr
	   << " yErr: " << *yErr << " zErr:" << *zErr;
	STDLINE(ss.str(), ACGreen);

	this->flipPositionLocal(x, y, xErr, yErr);
	ss.str("");
	ss << __PRETTY_FUNCTION__ << "After flipping ->"
	   << " x: " << *x << " y: " << *y << " z:" << *z << " xErr: " << *xErr
	   << " yErr: " << *yErr << " zErr:" << *zErr;
	//    STDLINE(ss.str(),ACRed);
	...
	this->translateXY(x, y);
	ss.str("");
	ss << __PRETTY_FUNCTION__ << "After translate->"
	   << " x: " << *x << " y: " << *y << " z:" << *z << " xErr: " << *xErr
	   << " yErr: " << *yErr << " zErr:" << *zErr;
	//   STDLINE(ss.str(),ACRed);
	...
	this->translateCorrection(x, y);
	ss.str("");
	ss << __PRETTY_FUNCTION__ << "After tran corr->"
	   << " x: " << *x << " y: " << *y << " z:" << *z << " xErr: " << *xErr
	   << " yErr: " << *yErr << " zErr:" << *zErr;
	//   STDLINE(ss.str(),ACGreen);
	...
	this->XRotation(y, z, yErr, zErr);
	ss.str("");
	ss << __PRETTY_FUNCTION__ << "After XRotation->"
	   << " x: " << *x << " y: " << *y << " z:" << *z << " xErr: " << *xErr
	   << " yErr: " << *yErr << " zErr:" << *zErr;
	//    STDLINE(ss.str(),ACRed);
	...
	this->YRotation(x, z, xErr, zErr);
	ss.str("");
	ss << __PRETTY_FUNCTION__ << "After YRotation->"
	   << " x: " << *x << " y: " << *y << " z:" << *z << " xErr: " << *xErr
	   << " yErr: " << *yErr << " zErr:" << *zErr;
	STDLINE(ss.str(), ACRed);
	...
	this->ZRotation(x, y, xErr, yErr);
	ss.str("");
	ss << __PRETTY_FUNCTION__ << "After ZRotation->"
	   << " x: " << *x << " y: " << *y << " z:" << *z << " xErr: " << *xErr
	   << " yErr: " << *yErr << " zErr:" << *zErr;
	//    STDLINE(ss.str(),ACRed);
	...
	ss.str("");
	ss << __PRETTY_FUNCTION__ << "After XYZRot   ->"
	   << " x: " << xc << " y: " << yc << " z:" << zc << " xErr: " << xErrc
	   << " yErr: " << yErrc << " zErr:" << zErrc;
	//    STDLINE(ss.str(),ACRed);

	*z += this->getZPositionTotal();

	ss.str("");
	ss << __PRETTY_FUNCTION__ << "Final          ->"
	   << " x: " << *x << " y: " << *y << " z:" << *z << " xErr: " << *xErr
	   << " yErr: " << *yErr << " zErr:" << *zErr;
	STDLINE(ss.str(), ACPurple);
	...
	*z = 0;
	this->flipBackPositionLocal(x, y, xErr, yErr);
	ss.str("");
	ss << __PRETTY_FUNCTION__ << "Final AfterFlip->"
	   << " x: " << *x << " y: " << *y << " z:" << *z << " xErr: " << *xErr
	   << " yErr: " << *yErr << " zErr:" << *zErr;
	STDLINE(ss.str(), ACGreen);
	...
	ss.str("");
	ss << __PRETTY_FUNCTION__ << "After DetToGlob->"
	   << " x: " << *x << " y: " << *y << " z:" << *z << " xErr: " << *xErr
	   << " yErr: " << *yErr << " zErr:" << *zErr;
	STDLINE(ss.str(), ACPurple);
	...
	ss.str("");
	ss << __PRETTY_FUNCTION__ << "After GlobToDet->"
	   << " x: " << *x << " y: " << *y << " z:" << *z << " xErr: " << *xErr
	   << " yErr: " << *yErr << " zErr:" << *zErr;
	STDLINE(ss.str(), ACGreen);
}
...
void Detector::dump(void)
{
	char buffer[128];
	sprintf(buffer,
	        "Alpha -> Total = %10.2f (%10.2f + %7.2f) deg",
	        getXRotation() + getXRotationCorrection(),
	        getXRotation(),
	        getXRotationCorrection());
	STDLINE(buffer, ACCyan);
	sprintf(buffer,
	        "Beta  -> Total = %10.2f (%10.2f + %7.2f) deg",
	        getYRotation() + getYRotationCorrection(),
	        getYRotation(),
	        getYRotationCorrection());
	STDLINE(buffer, ACCyan);
	sprintf(buffer,
	        "Gamma -> Total = %10.2f (%10.2f + %7.2f) deg",
	        getZRotation() + getZRotationCorrection(),
	        getZRotation(),
	        getZRotationCorrection());
	STDLINE(buffer, ACCyan);
	sprintf(buffer,
	        "X     -> Total = %10.2f (%10.2f + %7.2f) um",
	        10 * (getXPositionTotal()),
	        10 * getXPosition(),
	        10 * getXPositionCorrection());
	STDLINE(buffer, ACCyan);
	sprintf(buffer,
	        "Y     -> Total = %10.2f (%10.2f + %7.2f) um",
	        10 * (getYPositionTotal()),
	        10 * getYPosition(),
	        10 * getYPositionCorrection());
	STDLINE(buffer, ACCyan);
	sprintf(buffer,
	        "Z     -> Total = %10.2f (%10.2f + %7.2f) um",
	        10 * (getZPositionTotal()),
	        10 * getZPosition(),
	        10 * getZPositionCorrection());
	STDLINE(buffer, ACCyan);
}
