...
{
	for(Event::alignedHitsCandidateMapDef::iterator pit =
	        trackCandidates_[trackNum].begin();
	    pit != trackCandidates_[trackNum].end();
	    pit++)
	{
		double resX =
		    ((*pit).second["x"] - fittedTracks_[trackNum][0] * (*pit).second["z"] -
		     fittedTracks_[trackNum][1]);
		double resY =
		    ((*pit).second["y"] - fittedTracks_[trackNum][2] * (*pit).second["z"] -
		     fittedTracks_[trackNum][3]);
...
//================================================================
void Event::setClusters(const clustersMapDef& clusters) { clusters_ = clusters; }
                                        ...
                                        double           fittedTrackChi2)
{
	while(unconstrainedFittedTracks_.size() <= trackN)
	{
		...
		unconstrainedFittedTracksCovariance_.push_back(
		    std::map<std::string, matrixDef>());
